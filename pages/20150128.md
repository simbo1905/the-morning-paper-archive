---
blog_url: https://blog.acolyer.org/2015/01/28/eraser-a-dynamic-data-race-detector-for-multi-threaded-programs/
blog_title: Eraser: A dynamic data race detector for multi-threaded&nbsp;programs
---
...
- Memory reuse: if a program allocates its own memory blocks and later on privately recycles them, Eraser has no way of knowing that the 'new' memory is now protected by a new set of locks.
...
- Benign race: genuine races, but which do not affect the result of the program (some of these may be intentional)
...
## Post navigation
 ...
 &lsaquo; PreviousZooKeeper: wait-free coordination for internet scale systemsNext &rsaquo;On Distributed Communications Networks
