---
blog_url: https://blog.acolyer.org/2015/11/16/simplifying-and-isolating-failure-inducing-input/
blog_title: Simplifying and Isolating Failure-Inducing Input
---
...
- Partition the failing input (sequence of changes) into two subsets with similar sizes, lets call them &Delta;1 and &Delta;2 ( reducing with n=2 subsets)

- Test &Delta;1 and &Delta;2

- If &Delta;1 reproduces the failure, we can reduce to &Delta;1 and continue the search in this subset. We can do likewise with &Delta;2 if that reproduces the failure.

But what if neither &Delta;1 nor &Delta;2 reproduces the failure? Perhaps both tests pass, or perhaps they result in input that is not well-formed from the perspective of the program under test and cause the test to be 'unresolved.' We have two possible strategies open to us:
...
- Partition the failing input into a larger number of subsets (twice as many as on the previous attempt). Test each small subset &Delta;i, as well as its (large) complement: failing input - &Delta;i. From testing each &Delta;i and its complement we get four possible outcomes:

- If the test of any &Delta;i reproduces the failure, then we can reduce to this subset and continue with this as our new input (reducing with n=2 subsets).

- If the test of any &Delta;i complement reproduces the failure, then continue from this complement and reduce it with n-1 subsets.
...
ddmin guarantees to eventually return a 1-minimal test case. If there are c changes in the original failing test case then in the worst case ddmin takes c2 + 3c tests (it will most likely take much less than this, due to the divide-and-conquer nature of the algorithm). If there is only one failure inducing change somewhere in the input, and all tests cases that include it reproduce the failure then the number of tests needed to find that change will be &leq; _2log2c.
...
When computing subsets, instead of computing subsets simply of the failing test case, compute subsets &Delta; of the difference between the passing set and the failing set. Now we add two new rules:

- Test the passing set &Union; &Delta;i for each &Delta;i. If we find a combination that passes then we have a larger passing test case and we can continue to the next iteration (reducing the difference between the new passing test case and the failing case).

- If none of these pass then test the failing set - &Delta;i for each &Delta;i. If we find a combination that passes then once again we have a larger passing test case...
...
## Post navigation
 ...
 &lsaquo; PreviousScrap Your Boilerplate with Object AlgebrasNext &rsaquo;Hierarchical Delta Debugging
...
## 
 3 thoughts on &ldquo;Simplifying and Isolating Failure-Inducing Input&rdquo; 
...
- 
 ...
 Pingback: Hierarchical Delta Debugging | the morning paper 
 
 
- 
 
 Pingback: Minimizing Faulty Executions of Distributed Systems | the morning paper 
