---
blog_url: https://blog.acolyer.org/2015/10/08/lprof-a-non-intrusive-request-flow-profiler-for-distributed-systems/
blog_title: lprof: A Non-intrusive Request-Flow Profiler for Distributed&nbsp;Systems
---
...
- Top-level methods: a list of tuples with (i) the name of the top-level method, (ii) an index into the DAG representation of the log points, and (iii) a list of request identifiers;

- DAGs: the DAG for each top-level method;

- Log point regex: the regular expressions for each log point and the identifier for each wildcard;

- Communication pairs: a list of tuples that identify the communication points along with the identifiers for the data being communicated.
...
## Post navigation
 ...
 &lsaquo; PreviousThe Mystery Machine: End-to-end performance analysis of large-scale internet servicesNext &rsaquo;App-Bisect: Autonomous healing for microservices-based apps
...
## 
 6 thoughts on &ldquo;lprof: A Non-intrusive Request-Flow Profiler for Distributed Systems&rdquo; 
...
- 
 ...
 Pingback: Dependency-driven analytics: a compass for uncharted data oceans | the morning paper 
...
hi! thank you for this paper. I have just not understand why in a system we are not able to create unique request identifiers to gather all the logs. Best regards, Alex
