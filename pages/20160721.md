---
blog_url: https://blog.acolyer.org/2016/07/21/time-adaptive-sketches-ada-sketches-for-summarizing-data-streams/
blog_title: Time-adaptive sketches (Ada sketches) for summarizing data&nbsp;streams
---
...
On querying of course, the de-emphasis process must be applied, which means dividing the results by f(t) to obtain the estimate of item i at time t. In the absence of collisions, as with the base CMS, counts are estimated exactly. Consider a CMS with only one row, and the case when two independent items i and j collide. We see cit instances of i, and cjt' instances of j. With plain CMS, we would over-estimate the count for i by cjt', whereas with the pre-emphasis process we overestimate by (f(t) x cjt)/f(t')). Therefore it is easy to see that more recent items suffer less compared to older items.
...
One major question we are interested in is "Given a fixed space and current state of time T, what are the values of time t &le; T where Ada-CMS is more accurate than vanilla CMS?
...
To illustrate a reasonable scenario, suppose we want the errors with Ada-CMS to be never off by a factor γ away from that of vanilla CMS &forall; t. This ensures that we guarantee accuracy within a factor γ of what the original CMS would achieve to even very old heavy hitters. In addition, we want to be more accurate than CMS on all recent time t > K, for some desirable choice of K.
...
## Post navigation
 ...
 &lsaquo; PreviousRange thresholding on streamsNext &rsaquo;End of Term, and the power of compound interest
...
## 
 2 thoughts on &ldquo;Time-adaptive sketches (Ada sketches) for summarizing data streams&rdquo; 
...
- 
 ...
 Pingback: Time-adaptive sketches (Ada sketches) for summarizing data streams | Simply Bytes 
