---
blog_url: https://blog.acolyer.org/2018/08/17/fairness-without-demographics-in-repeated-loss-minimization/
blog_title: Fairness without demographics in repeated loss&nbsp;minimization
---
...
We have a population with latent groups. Each group is a proportion of the overall population, and has a distribution . We have some trained model with parameters used to make predictions based on an input arising from one of the latent groups. When a user makes a query, they incur some expected loss, which is termed the risk in this paper.

Representation disparity occurs when there is a meaningful gap between the overall risk and the maximum (worst-case) group risk . I.e., good overall performance, but poor performance (higher expected loss) for the worst-case group in K. In the evaluation example, the task is text prediction for autocomplete and the training data set contains a mix of African-American English (AAE) and Standard-American English (SAE) tweets. If the AAE data is in the minority, we would expect worse than average model performance when autocompleting African-American English. The measure of the representation disparity is given by .
...
To keep a handle on the representation disparity, we want to control (minimize) the worst-case group risk over all time periods in .
...
For a system to be fair over the long term there needs to be a stable and fair fixed-point for the expected user counts over time. That is, risk minimisation needs to maintain the same population fraction over time. It’s easy to violate this condition when minimising overall risk (the standard empirical risk minimization, ERM, approach), even when we start out with two groups that have identical population size and initial risk.

In the following example we have two initially equal groups (one on the left, one on the right), and a classifier tasked with classifying points as red or black. Start out with a classifier with boundary (optimal for this data set). Now let optimisation play out over rounds. At some point in the earlier rounds, due to random fluctuations in sampling accuracy, we took slightly fewer samples from the cluster on the right, meaning that ERM improves the loss for the left cluster at the expense of the right-hand one (see e.g. the line). This increases churn in the right-hand cluster, reducing its representation in future rounds. Once the scales have tipped even slightly like this the disparity is amplified until at there are nearly no samples from the right-hand cluster and it ends up suffering high loss.
...
In this case, what the authors do is consider all possible distributions within a distance r of the overall population distribution , where r is the chi-squared divergence measure between the two distributions. The result is a ‘chi-squared ball’. Let be the worst-case loss over all the r-perturbations around P. The authors show that this bounds the risk of the group and hence the overall worst-case risk.

If the smallest minority group proportion we want to consider is , then we can control the worst-case group risk by minimizing the upper bound of with . See §4.1 in the paper for details. Under the covers what happens is that small levels of loss get ignored, and large losses above a threshold are upweighted due to a square term. My statistics knowledge isn’t good enough to take you any deeper here!
...
> 
 ...
 For both ERM and DRO, we train a set of five maximum likelihood bigram language models on a corpus with 366,361 tweets total and a fraction of the tweets labelled as AAE. This results in 10 possible autocomplete systems a given Mechanical Turk user can be assigned to during a task.
...
## Post navigation
 ...
 &lsaquo; PreviousObfuscated gradients give a false sense of security: circumventing defenses to adversarial examplesNext &rsaquo;Filter before you parse: faster analytics on raw data with Sparser
...
## 
 5 thoughts on &ldquo;Fairness without demographics in repeated loss minimization&rdquo; 
...
- 
 ...
 Pingback: Four short links: 22 August 2018 | Crypto War - Bitcoin, Cryptocurrency, News Blog 
 
 
- 
 
 Pingback: Four short links: 22 August 2018 - Home Improvement Designs 
 
 
- 
 
 Pingback: Four short links: 22 August 2018 - DUI Lawyer 
 
 
- 
 
 Pingback: Four short links: 22 August 2018 - Tesco Inc. Online Electronics Store 
