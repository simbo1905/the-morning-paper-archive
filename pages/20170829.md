---
blog_url: https://blog.acolyer.org/2017/08/29/seeing-is-believing-a-client-centric-specification-of-database-isolation/
...
Let a storage system state be a mapping from a set of keys to a set of values . Initially the system starts out with every key mapped to the special undefined value, 

A transaction is an ordered set of read and write operations. A read operation retrieves the values by reading key . A write operation updates to the new value .

If we start with the system in some state (called the parent state of a transaction) and apply a transaction , we end up with a new state in which the only differences are that the writes from T have been applied. (For simplicity, a transaction is assumed to write a key only once). In the notation developed in the paper, this is written as . A series of zero or more such state transitions from an initial starting state is written .

So far so good. Now we need to grapple with the concept of an execution. Given a set of transactions, , an execution is the sequence of states that the system passes through (starting from the initial state) when applying the transactions in in some order. So one execution might be “we start out in the initial state and then apply , , and in that order,” and other execution for the same transaction set might be “we start out in the initial state and then apply , , in that order.”

Here’s the subtle thing it took me a few readings to fully grasp. Take the order , , again for a moment. There is no requirement that the state a given transaction sees be the immediately preceding state in the execution. So for it could be seeing the initial state, the state after applying , or the state after applying and . Since a read operation is defined in terms of the value that is seen in response to a read of the key we do have some constraints: the value returned from the read must be compatible with the value of in the state from which the operation reads. The subset of states in the execution for which this condition holds are called the read states. To put it another way, the read states of an operation are all of the states it could possibly have read from given the value that it actually observed.
...
For this execution, we’ll say that is applied first. Since it can only read from the initial state, we inevitably end up with:

 goes next, taking us to state :

and then it’s the turn of :

(Note that these first three transactions handily didn’t include any read operations!). Next up in the execution is . This does contain read operations, two of them. Operation reads key and sees . The only possible read state for therefore is .

Operation reads key and sees value . The possible read states for this operation (states in which has value ) are therefore:

The read precondition predicate, PREREAD, holds for a transaction if all the read operations in the transaction have a non-empty read set. That is, every read seen by the transaction was at least possible according to our rules so far! In the example above, holds since the read set is non-empty for both reads. The PREREAD predicate holds for an entire execution if it holds for every transaction in the execution.

For a given transaction , a state in an execution is said to be complete if every operation in can read from (i.e., is in the read set of every operation in ). We denote this as . In our running example, the only complete state for transaction is . There is no guarantee that a complete state will exist for any given transaction.
...
- Every operation in must read from the same state (i.e., a state that is complete for ).

- This complete state must also be the parent state for (i.e., the state that transitions from). 

If we want strict serializability we need to add one more commit test condition. We say that transaction precedes if the commit time of is less than the start time of (i.e, commits before starts). The extra condition is this: if precedes then comes before in the execution.
...
- Every operation in must read from the same state (i.e., a state that is complete for ). This is the same condition as we had for serializable.

- cannot modify (write) any keys that changed value between the complete state that reads from, and the actual parent state of .

Going back to our example, is a complete state for . Under snapshot isolation we would be allowed to extend our execution with following (having as its parent state) , but under serializability we would not.
...
## Post navigation
 ...
 &lsaquo; PreviousUsing chatbots against voicespam: analyzing Lenny's effectivenessNext &rsaquo;A concurrent perspective on smart contracts
