---
blog_url: https://blog.acolyer.org/2018/01/31/a-theory-of-the-learnable/
blog_title: A theory of the&nbsp;learnable
---
...
- The machines can provably learn whole classes of concepts. Furthermore, these classes can be characterised.
...
With no loss of generality, Valiant considers learning functions of boolean variables. There is no assumption of independence between these variables. I can encode anything I like that way. For example, I can have a boolean variable which indicates whether or not a certain input feature has the value “3”. I can also have a boolean variable that is a function of other variables. If we have t such Boolean variables, then there are combinations. There is a function f from these combinations to {0,1} (indicating whether or not a particular combination is an instance of the category we want to learn to detect).

Now consider a function F whose domain is a superset of f’s, and for which . (I.e., it gives us some additional variables on which f does not depend). This function F is what Valiant calls a concept.

Underlying the concept F is a probability distribution D over the set of all input vectors v. For every v such that latex , .
...
- The algorithm runs in polynomial time in an adjustable parameter h, in the various parameters that quantify the size of the program to be learned, and in the number of variables t.

- For all programs and all distributions D over vectors v on which f outputs 1, the algorithm will deduce with probability at least a program that never outputs one when it should not but outputs one almost always when it should. (The sum of D(v) over all v where and is at most ).
...
Valiant gives results for conjunctive normal-form (CNF) expressions, disjunction normal-form (DNF) expressions, and arbitrary expressions in which each variable occurs just once. In all cases the probabilistic analysis depends on a lemma for a function defined for any real number h greater than one, and any positive integer S.

> 

 Let be the smallest integer such that in independent Bernoulli trials each with probability at least of success, the probability of having fewer than S successes is less than ... The following simple upper bound holds for the whole range of values of S and h and shows that is essentially linear both in h and in S.
...
> 
 ...
 For any positive integer k, the class of k-CNF expressions is learnable via an algorithm A that uses calls of EXAMPLES and no calls of ORACLE, where t is the number of variables.
...
> 
 ...
 The class of monotone DNF expressions is learnable via an algorithm B that uses calls of EXAMPLES and calls of ORACLE, where d is the degree of the DNF expression f to be learned and t the number of variables.

(The algorithm begins with an ‘empty’ function g , and adds just enough to g with each successive example to make ).
...
Given a slightly more powerful oracle (that can answer questions of the form “does there exist an input vector w such that...”), μ-expressions are learnable in calls to the oracle.
...
## Related
 ...
 AIGreat momentsMachine Learning 
...
## Post navigation
 ...
 &lsaquo; PreviousA sample of brillianceNext &rsaquo;Learning representations by back-propagating errors
...
## 
 7 thoughts on &ldquo;A theory of the learnable&rdquo; 
...
- 
 ...
 Pingback: New top story on Hacker News: A theory of the learnable - NEWS TODAY 
 
 
- 
 
 Pingback: New top story on Hacker News: A theory of the learnable - Tech + Hckr News 
 
 
- 
 
 Pingback: A theory of the learnable - posted at January 31, 2018 at 10:28AM by mpweiher - Startup News 2018 
 
 
- 
 
 Pingback: New top story on Hacker News: A theory of the learnable - ÇlusterAssets Inc., 
 
 
- 
 
 Pingback: Four short links: 1 February 2018 - Home Improvement Designs 
 
 
- 
 
 Pingback: Four short links: 1 February 2018 - Site Title 
 
 
- 
 
 Pingback: Four short links: 1 February 2018 - DUI Lawyer 
