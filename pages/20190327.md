---
blog_url: https://blog.acolyer.org/2019/03/27/amazon-aurora-on-avoiding-distributed-consensus-for-i-os-commits-and-membership-changes/
blog_title: Amazon Aurora: on avoiding distributed consensus for I/Os, commits, and membership&nbsp;changes
---
...
## Post navigation
 ...
 &lsaquo; PreviousAmazon Aurora: design considerations for high throughput cloud-native relational databasesNext &rsaquo;Calvin: fast distributed transactions for partitioned database systems
...
## 
 4 thoughts on &ldquo;Amazon Aurora: on avoiding distributed consensus for I/Os, commits, and membership changes&rdquo; 
...
How do you prevent a node from serving a page that isnt durable? Without a read quorum you could only do that by serving pages less than the PGCL .. the node serving the page has to know what that is.
...
Hi, this is Anurag Gupta. I used to work on Aurora (really from conception to the end of last year). You're right that you should only serve pages below PGCL. Note that an individual storage node does not have a concept of PGCL - that would require a lot of traffic with its peers. Aurora counts on the database head node to maintain this information. To answer your second question, this is done by sending the desired read LSN with the page read request. A particular storage node might, as you point out, have a later copy of a page but is also guaranteed to have not GC'ed earlier copies of pages until it is certain that all database head nodes have advanced their minimum read point. This is needed to consistently serve pages to read replicas that might be slightly in the past of the master node. 

Sorry for the late reply to your question. I only just happened upon this blog while googling for something.
