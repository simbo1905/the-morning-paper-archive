---
blog_url: https://blog.acolyer.org/2017/09/14/accelerating-innovation-through-analogy-mining/
blog_title: Accelerating innovation through analogy&nbsp;mining
---
...
Suppose for each product we had purpose and mechanism vectors , , and we also have distance metrics , between purpose and mechanism vectors respectively.

Then we could for example search for products that have the same (very similar) purpose, but a different mechanism. We want to find a product with the most similar purpose, , subject to the mechanism being sufficiently different: .

We could also search for products using the same (very similar) mechanism, but for a different purpose. We want to find a product with a similar mechanism, subject to the purpose being sufficiently different: .
...
Repeat the above process for each of the N training examples, and you end up with a set of N target tuples . We want to learn a function that predicts given as input a variable length sequence comprising the GloVe word vectors for each word in the product description.

This function is learned using a bi-direction RNN passing forward and backward over the word vector sequence . If and are purpose and weight vectors, and is the hidden representation, then:
...
The first step is to run a k-means clustering algorithm over the purpose vectors, and then prune away the smaller clusters. The remaining clusters are ranked by an intra-distance measure and the top are selected. For each of these top purpose clusters, select the product with purpose vector closest to the centre of the cluster. Use this product as the kth seed product, .
...
## Post navigation
 ...
 &lsaquo; PreviousAdversarial examples for evaluating reading comprehension systemsNext &rsaquo;Struc2vec: learning node representations from structural identity
...
## 
 One thought on &ldquo;Accelerating innovation through analogy mining&rdquo; 
