---
blog_url: https://blog.acolyer.org/2018/10/05/i-know-youll-be-back-interpretable-new-user-clustering-and-churn-prediction-on-a-mobile-social-application/
blog_title: I know you’ll be back: interpretable new user clustering and churn prediction on a mobile social&nbsp;application
---
...
The daily activity metrics are all over the place (e.g, see below for a plot of ‘chat received’ counts). So to make things more manageable two parameters are derived: is the mean of the daily measures, capturing activity volume, and is the lag(1) of the daily measures capturing burstiness ("both measures are commonly used in time series analysis").
...
These can be modelled by a sigmoid function with parameters and capturing the shape of the curve.

For each of the 12 attributes, we can therefore compute the four parameters giving us a 12&#215;4 feature matrix.
...
- For each user, we derive a new feature combination vector where each element is the cluster centre of the cluster the user belongs to for that feature. (E.g. if the user is in cluster 1 for the first feature, with cluster centre , and in cluster 3 for the second feature, with cluster centre , then the first two elements of the feature combination vector will be ). The purpose of this step is to reduce the influence of noise and outliers.
...
## Post navigation
 ...
 &lsaquo; PreviousCustomized regression model for Airbnb dynamic pricingNext &rsaquo;Online parameter selection for web-based ranking problems
...
## 
 2 thoughts on &ldquo;I know you’ll be back: interpretable new user clustering and churn prediction on a mobile social application&rdquo; 
...
- 
 ...
 Pingback: I know you’ll be back: interpretable new user clustering and churn prediction on a mobile social application - Charlescearl&#039;s Weblog 
 
 
- 
 
 Pingback: Data Science newsletter - October 10, 2018 | Sports.BradStenger.com 
