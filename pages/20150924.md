---
blog_url: https://blog.acolyer.org/2015/09/24/a-new-metaheuristic-bat-inspired-algorithm/
blog_title: A New Metaheuristic Bat-Inspired&nbsp;Algorithm
---
...
- They emit pulses at a fixed wavelength &lambda;, with varying frequency f and loudness A to search for prey.

- The rate of pulse emission r &isin; [0,1] varies depending on the proximity of the target
...
Starting out with an initial population of bats spread over the solution space, the algorithm proceeds in iterations. Each bat is randomly assigned a start frequency drawn from [fmin,fmax] - for example, 0..100. Each bat is also given a random initial loudness and pulse emission rate r &isin; [0,1] close to zero.
...
- fi = fmin + &beta;(fmax - fmin) , where &beta; is a random number between 0 and 1.
...
- xi = xG + &epsilon;A , where A is the average loudness of all bats at this time, and &epsilon; is a random number drawn from [-1,1].
...
- rit+1 = ri0[1 - exp(-&lambda;t)], where t represents the time step (iteration)

&alpha; and &lambda; are constants, both set to 0.9 in the simulation in the paper.
...
## Post navigation
 ...
 &lsaquo; PreviousFirefly Algorithms for Multimodal OptimizationNext &rsaquo;Cuckoo Search via Lévy Flights
