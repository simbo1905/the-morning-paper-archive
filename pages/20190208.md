---
blog_url: https://blog.acolyer.org/2019/02/08/graph-neural-networks-a-review-of-methods-and-applications/
blog_title: Graph neural networks: a review of methods and&nbsp;applications
---
...
> 
 The target of GNN is to learn a state embedding which contains the information of the neighbourhood for each node. 

Given the state embedding we can produce a node-level output , for example, a label. 

The local transition function, , updates a node’s state based on its input neighbourhood, and the local output function, , describes how the output is produced. We can express and in terms of and :

- .
- 
Where are the features of node , and are the features of its edges. and are the state and node features for the neighbourhood of . 

If we stack all the embedded states into one vector, , the outputs into one vector , the node and edge features into one vector and the just the node features into then we have:
...
The original GNN iterates to a fix point using .
...
## Post navigation
 ...
 &lsaquo; PreviousA comprehensive survey on graph neural networksNext &rsaquo;ExFaKT: a framework for explaining facts over knowledge graphs and text
