---
blog_url: https://blog.acolyer.org/2019/11/18/formal-foundations-of-serverless-computing/
blog_title: Formal foundations of serverless&nbsp;computing
---
...
## Lambda operational semantics
 ...
 is an operational semantics that captures these subtleties of a serverless platform. A serverless function itself is defined by three functions:

- init defines the initial state
- recv defines the behaviour upon receipt of a request, and
- step defines a processing step taken by the function that may produce a command for the serverless platform to execute. The only valid command at this point in our journey is return.
...
> 
First, we define a simpler naive semantics of serverless computing that eliminates most unintuitive behaviors of . Second, using a weak bisimulation theorem we precisely characterize when the naive semantics and coincide. This theorem address the low-level details of serverless execution once and for all, thus allowing programmers to reason using the naive semantics, even when their code is running on a full-fledged serverless platform.
...
- The serverless function is deterministic in its outputs: it produces the same observable command (if any) on any equivalent states
- All final states are equivalent to the initial state 

Note that a single naive state may be equivalent to multiple distinct states, and vice-versa. Given the above conditions, the weak bisimulation theorem in section 4 of the paper allows programmers to justifiably ignore the low-level details of the platform.
...
- Run two sub-programs in a sequence
- Running a sub-program on the first component of a tuple, and returning the second component unchanged.
...
## Post navigation
 ...
 &lsaquo; PreviousTaiji: managing global user traffic for large-scale Internet services at the edgeNext &rsaquo;Local-first software: you own your data, in spite of the cloud
...
## 
 2 thoughts on &ldquo;Formal foundations of serverless computing&rdquo; 
...
Thank you for the writeup! I do want to point out that the "composition property" of SPL, that the post mentions at the very end, comes from Apache OpenWhisk, and is studied in more depth in this Onward! paper:

https://dl.acm.org/citation.cfm?id=3133855
...
The example of ‘doing it wrong’ that you quote in the first code snippet, is so redacted in your version that it makes little sense. Could you expand it a little to make the ‘faults’ appear more compelling?

Just re-reading this one as a reference from a much later Morning Paper review.
