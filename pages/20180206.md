---
blog_url: https://blog.acolyer.org/2018/02/06/quantum-algorithms-an-overview/
blog_title: Quantum algorithms: an&nbsp;overview
---
...
For Shor’s algorithm, . But if we use other groups G, the same quantum HSP algorithm can also attack other cryptosystems:
...
A classical algorithm must evaluate f times in the worst case, but Grover’s quantum algorithm solves the problem using evaluations of f in the worst case. It is a bounded-error algorithm: it fails with probability for some arbitrarily small (but fixed) greater than 0. Grover’s algorithm does not rely on or exploit any internal structure in the function f.

Grover’s algorithm can be applied to any problem in the complexity class NP, since we can efficiently check if we have a solution in polynomial time (i.e., check whether ). In the following, we present a certificate which is a candidate proof of a solution, and the certificate can be verified in polynomial time:

> 

 Given a problem in NP that has a certificate length m, by applying Grover’s algorithm to A (the classical checking algorithm) and searching over all possible certificates, we obtain an algorithm which uses time rather than the used by classical exhaustive search over all certificates. This (nearly) quadratic speedup is less dramatic than the super-polynomial speedup achieved by Shor’s algorithm, but can still be rather substantial.
...
The classical solution can find an answer in an average of evaluations of f, simply by repeatedly running A and checking the output each time. A quantum algorithm known as amplitude amplification, due to Brassard et al., can find with only uses of f, and a failure probability arbitrarily close to 0 — a quadratic speedup.

3-SAT is an NP-complete problem that runs in time . Applying amplitude amplification to the problem gives a quantum algorithm with runtime , “illustrating that quantum computers can speed-up non-trivial classical algorithms for NP-complete problems.”
...
For example: finding the minimum of an unsorted list of N integers (quantum computers can do this in ); determining graph connectivity, and pattern matching (find a pattern P of length M within a text T of length N).

If we can find the minimum of an unsorted list of N integers in , we can also find the minimum of an arbitrary and initially unknown function with evaluations of f.
...
- faster hitting: the time it takes to find a target vertex from a source vertex (for some graphs, this time can be exponentially less), and

- faster mixing: the time taken to spread out over all vertices after starting from one source vertex. The mixing speedup can be quadratic, but no more than this. 

One application of quantum walks is in fast evaluation of boolean formulae - a formula on N binary inputs can be evaluated in ‘slightly more than’ operations (vs in the worst case for classical algorithms).
...
## Post navigation
 ...
 &lsaquo; PreviousQuantum computing in the NISQ era and beyondNext &rsaquo;Blockstack: a global naming and storage system secured by blockchain
...
## 
 13 thoughts on &ldquo;Quantum algorithms: an overview&rdquo; 
...
- 
 ...
 Pingback: New top story on Hacker News: Quantum algorithms: an overview - Tech + Hckr News 
 
 
- 
 
 Pingback: New top story on Hacker News: Quantum algorithms: an overview - ÇlusterAssets Inc., 
 
 
- 
 
 Pingback: Quantum algorithms: an overview - posted at February 06, 2018 at 09:13AM by godelmachine - Startup News 2018 
...
> Grover’s quantum algorithm solves the problem using $O(\sqrt{N})$ evaluations of f in the worst case.

Please explain what $N$ is in this expression.
...
Suffering from mixed capitalisation here! N is ‘n’ in the box above, the number of bits in the input.
...
- 
 ...
 February 6, 2018 at 6:04 pm 

 

 
 
Although I'm not an expert in quantum computing, every other resource I've read says that Grover's Algorithm provides quadratic speedup, not exponential. In other words, N=2^n, and Grover's Algorithm can search n bits in O(sqrt(2^n)) time.
...
Grover’s is definitely quadratic. If i said exponential that was a mistake! I will check the post and update as needed when I’m back at my desk. Thanks
...
interesting article! i will definitely look into this research path
...
- 
 ...
 Pingback: Four short links: 8 February 2018 | Vedalgo 
 
 
- 
 
 Pingback: Four short links: 8 February 2018 - Site Title 
 
 
- 
 
 Pingback: Four short links: 8 February 2018 - Home Improvement Designs 
 
 
- 
 
 Pingback: Four short links: 8 February 2018 - DUI Lawyer 
...
That's clearly good explanation👍👍
