---
blog_url: https://blog.acolyer.org/2019/12/02/efficient-lock-free-durable-sets/
blog_title: Efficient lock-free durable&nbsp;sets
---
...
The paper presents two implementations of this idea: link-free sets is a fairly straightforward implementation of the basic idea; whereas SOFT (Sets with an Optimal Flushing Technique) further reduces the number of fences to the theoretical minimum — at the expense of algorithmic complexity.
...
- A new node is initialised, with its validity set to false

- The key and value are written into the new node

- The node is inserted into the linked list structure, and the validity bit is flipped

- A pysnc operation makes the node durable
...
SOFT reduces the number of psync operations required to the theoretical lower bound, by dividing each update operation into two stages: intention and completion.
...
Both the link-free algorithm and SOFT need a way to reclaim memory. There are lock-free algorithms for this, but none quite fit the bill. Instead the authors use the Epoch Based Reclamation scheme (EBR). EBR is not lock-free, but offers good performance and provides progress for memory management when threads are not stuck.
...
## Post navigation
 ...
 &lsaquo; PreviousTLA+ model checking made symbolicNext &rsaquo;Declarative assembly of web applications from pre-defined concepts
...
## 
 15 thoughts on &ldquo;Efficient lock-free durable sets&rdquo; 
...
It appears that ordering information is never persisted, since this is a set, and thus the linked list can be reconstructed in any order. Presumably other data structures that require ordering or other structure cannot be implemented as cheaply.
...
- 
 ...
 Pingback: New top story on Hacker News: Efficient lock-free durable sets - Golden News 
 
 
- 
 
 Pingback: New top story on Hacker News: Efficient lock-free durable sets - Outside The Know 
 
 
- 
 
 Pingback: New top story on Hacker News: Efficient lock-free durable sets - News about world 
 
 
- 
 
 Pingback: New top story on Hacker News: Efficient lock-free durable sets - protipsss 
 
 
- 
 
 Pingback: New top story on Hacker News: Efficient lock-free durable sets - Ultimate News 
 
 
- 
 
 Pingback: Efficient lock-free durable sets - TechBits 
 
 
- 
 
 Pingback: New top story on Hacker News: Efficient lock-free durable sets - Hckr News 
 
 
- 
 
 Pingback: Top story HACKER NEWS: Efficient lock-free durable sets - Nate&#039;s Blog 
 
 
- 
 
 Pingback: Efficient lock-free durable sets - INDIA NEWS 
 
 
- 
 
 Pingback: New top story on Hacker News: Efficient lock-free durable sets - Latest news 
 
 
- 
 
 Pingback: Efficient lock-free durable sets | Blog 
 
 
- 
 
 Pingback: Efficient lock-free durable sets - Hacker News Robot 
 
 
- 
 
 Pingback: A persistent problem: managing pointers in NVM - the morning paper 
 
 
- 
 
 Pingback: An empirical guide to the behavior and use of scalable persistent memory - the morning paper 
