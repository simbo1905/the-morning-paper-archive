---
blog_url: https://blog.acolyer.org/2017/02/15/redqueen-an-online-algorithm-for-smart-broadcasting-in-social-networks/
blog_title: RedQueen: An online algorithm for smart broadcasting in social&nbsp;networks
---
...
Consider a tweeting user (a broadcaster in the terminology used in the paper). Such a user can be modelled by a temporal point process, which is a fancy way of saying a stochastic process generating a sequence of discrete events (tweets) over time. If we just care about the number of tweets a user makes, we can simplify further and use a counting process representation: let be the number of tweets up to time . The number of tweets we'll see in that time depends of course on how actively the user tweets, which can in turn be modelled by an intensity function .

The intensity function used in the paper has two terms: the first term is a time-varying function modelling the publication of tweets by users on their own initiative; the second α-weighted term models the publication of additional messages (e.g., replies, retweets), by users due to the influence of previous messages.

> 
The second term makes the intensity dependent on history and a stochastic process by itself.

The combination of a counting process and an intensity function results in 'a doubly stochastic Markov process, whole dynamics can be defined by a jump SDE.'

A collection of broadcasting users is a vector in which the ith dimension is the number of tweets broadcast by user i up to time t. represents the times of the stories user j receives due to other twitter users she follows .
...
In the above, is a time significance function , which favours some periods of times (e.g., times in which the follower is online), and is a given parameter which trades-off between visibility and the number of broadcast posts.

The algorithm as defined above only deals with a single follower. To deal with n followers simply sum the term over all n.
...
## Related
 ...
 Algorithms and data structuresSocial Networks 
...
## Post navigation
 ...
 &lsaquo; PreviousReducing controversy by connecting opposing viewsNext &rsaquo;Beyond the words: predicting user personality from heterogeneous information
...
## 
 5 thoughts on &ldquo;RedQueen: An online algorithm for smart broadcasting in social networks&rdquo; 
...
- 
 ...
 Pingback: RedQueen: An online algorithm for smart broadcasting in social networks | Ace Infoway 
 
 
- 
 
 Pingback: Four short links: 15 Feb 2017 | Vedalgo 
...
Second callout: "makes the intensity *dependent* on history..." (not "dependency")
...
- 
 ...
 Pingback: Four short links: 15 Feb 2017 | A bunch of data 
