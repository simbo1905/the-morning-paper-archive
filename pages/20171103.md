---
blog_url: https://blog.acolyer.org/2017/11/03/log20-fully-automated-optimal-placement-of-log-printing-statements-under-specified-overhead-threshold/
blog_title: Log20: Fully automated optimal placement of log printing statements under specified overhead&nbsp;threshold
---
...
Log20 samples the production system to collect path profiles. Let be the number of occurrences of path divided by the total number of paths sampled in the system. In other words, is an estimate of the probability of observing execution path . Using Shannon’s entropy we can measure the overall unpredictability in the system as:

 

We instrument a subset of the blocks. When execution follows a given path, it produces a log sequence containing log entries for the instrumented blocks only. Given a log sequence and a placement of log statements, it’s possible therefore that multiple execution paths may give rise to the same log sequence. As a trivial example, suppose that in our placement we have just one LPS in the block on line 2 - then any of the paths through will result in the same log sequence.

Let the Possible Paths of a log sequence , be the set of paths that would output when executed.

Given a placement of log statements , then we can use entropy to model how much information we are getting from those log statements. Consider a particular log output , the entropy is:

 

Where is the probability of the program taking a path that outputs L. is therefore telling us the probability that we took path given that we saw , .

Now consider all possible log outputs produced by the placement . We can measure the entropy of the placement, as , where is the set of all possible log sequences under placement . This reduces to:

 

## What’s the overhead of a log statement?

If we assume a fixed cost each time we emit a log statement, then the cost of given log statement placement is directly proportional to the number of times we expect it to be executed. We can figure this out from the production sampling, and assign each placement a weight representing that cost.

## The Log20 placement algorithm

> 

 Given a set of basic blocks, BB, where each block has a weight, , the problem of placement is to find a subset of BB, , such that the sum of the weights of all basic blocks in is under a threshold, , and entropy is minimized.
...
## Post navigation
 ...
 &lsaquo; PreviousMy VM is lighter (and safer) than your containerNext &rsaquo;Statistically rigorous Java performance evaluation
...
## 
 15 thoughts on &ldquo;Log20: Fully automated optimal placement of log printing statements under specified overhead threshold&rdquo; 
...
- 
 ...
 Pingback: SRE Weekly Issue #96 - SRE WEEKLY 
...
- 
 ...
 Pingback: Log20: Fully automatic optimum placement of log printing statements under specified overhead threshold | A1A 
