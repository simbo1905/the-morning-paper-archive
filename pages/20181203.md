---
blog_url: https://blog.acolyer.org/2018/12/03/securify-practical-security-analysis-of-smart-contracts/
blog_title: Securify: practical security analysis of smart&nbsp;contracts
---
...
For the instructions that are left, Securify infers a set of base facts of the form where is the instruction name, is the instruction label, is the instruction storing the instruction result (if any), and are the arguments. In addition, Securify adds a follow fact, to indicate that the instruction with label immediately follows instruction . The joining of then/else branches is captured by a fact.
...
- Ether Liquidity (LQ). This property ensures that ether cannot get lost inside a contract. That is, there exists a trace that decreases the contracts balance. As an example violation, $160M was frozen in November 2017 when a contract relied on another contract to perform ether transfers (i.e., used it as a library), and then the library contract was ‘killed’ (removed from the chain). (Aside: the fact that this can happen seems to make it super-hard in general to write any contract that depends on some other library contract the author does not also control).

- No writes after calls (NW). This captures a scenario where a contract calls out to user controlled code, and that code is able to alter the state inside the contract before it returns. Such a bug in the DAO contract led to an attacker stealing $60M.

- Restricted Writes (RW). An attacker stole $30M by finding a route to change the owner field in a contract. This property guarantees that writes to storage are restricted.

- Restricted Transfer (RT). Detects Ponzi schemes by checking that ether transfers via call cannot be invoked by any user.

- Handled Exception (HE). This property checks that the return value of all call instructions is actually checked.
...
- Validate Arguments (VA). Checks that all method arguments are verified before being used/stored.
...
## Post navigation
 ...
 &lsaquo; PreviousLEMNA: explaining deep learning based security applicationsNext &rsaquo;FairSwap: how to fairly exchange digital goods
...
## 
 3 thoughts on &ldquo;Securify: practical security analysis of smart contracts&rdquo; 
...
The core code is also available there:
...
- 
 ...
 Pingback: MadMax: surviving out-of-gas conditions in Ethereum smart contracts | the morning paper 
 
 
- 
 
 Pingback: MadMax: surviving out-of-gas conditions in Ethereum smart contracts | Daily AI 
