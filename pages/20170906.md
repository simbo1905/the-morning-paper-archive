---
blog_url: https://blog.acolyer.org/2017/09/06/rem-resource-efficient-mining-for-blockchains/
blog_title: REM: Resource-efficient mining for&nbsp;blockchains
---
...
To protect against compromised chips, REM includes a block acceptance policy which adds an extra test on top of the instruction count lottery we've seen so far. is based on statistical testing. Starting with an estimate of the rate at which the fastest honest miners (fastest CPU type) can find valid PoUWs, test blocks statistically to determine whether a miner is mining blocks too quickly and thus may be compromised. Of course, an adversary with knowledge of can publish blocks at the fastest rate that it will accept. The algorithm can be tuned ($\alpha$) to trade-off between the amount of waste (falsely rejected blocks) and the advantage that such an adversary can gain.

With = 0.4 the waste level is 0.6%, and the adversaries advantage is capped at 100.6%.
...
## Post navigation
 ...
 &lsaquo; PreviousLuck is hard to beat: the difficulty of sports predictionNext &rsaquo;SmartPool: Practical decentralized mining
...
## 
 7 thoughts on &ldquo;REM: Resource-efficient mining for blockchains&rdquo; 
...
At least in its current revision (v2) IAS includes the request in its response: https://software.intel.com/sites/default/files/managed/7e/3b/ias-api-spec.pdf p24
...
http://www.truthcoin.info/blog/pow-cheapest/ explains (imo convincingly) how the idea of a PoW which isn't wasteful is a fallacy.

I think the problem here is that producing electricity has externalized costs (to the environment). Mining for crypto-currencies just makes this problem more obvious in an intuitive way.
...
If this consensus layer was added to a cryptocurrency with an incentive for doing so, wouldn't it incentivize miners to run meaningless programs constantly (perhaps programs that Intel chips are highly optimized for) thus creating a similar energy problem?
...
It depends on how optimized it can be and how much money miners can earn from doing useful work. For the second factor, ec2 pricing (https://aws.amazon.com/ec2/pricing/) is a reasonable estimation. The first factor is hard to quantitate though.
...
- 
 ...
 Pingback: El nuevo dinero también contamina - Leequid.es 
