---
blog_url: https://blog.acolyer.org/2014/12/10/why-events-are-a-bad-idea/
blog_title: Why events are a bad&nbsp;idea
---
...
> 
 ...
 Event-based programming has been highly touted in recent years as the best way to write highly concurrent applications. Having worked on several of these systems, we now believe this approach to be a mistake. Speciﬁcally, we believe that threads can achieve all of the strengths of events, including support for high concurrency, low overhead, and a simple concurrency model. Moreover, we argue that threads allow a simpler and more natural programming style.
...
> 
 ...
 For example, our own Ninja system [16] ended up using threads for the most complex parts, such as recovery, simply because it was nearly impossible to get correct behavior using events (which we tried ﬁrst). In addition, applications that didn’t need high concurrency were always written with threads, just because it was simpler.
...
## Post navigation
 ...
 &lsaquo; PreviousWhy threads are a bad ideaNext &rsaquo;A language-based approach to unifying events and threads
...
## 
 2 thoughts on &ldquo;Why events are a bad idea&rdquo; 
