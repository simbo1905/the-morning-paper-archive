---
blog_url: https://blog.acolyer.org/2016/11/17/twice-the-bits-twice-the-trouble-vulnerabilities-induced-by-migrating-to-64-bit-platforms/
blog_title: Twice the bits, twice the trouble: vulnerabilities induced by migrating to 64-bit&nbsp;platforms
---
...
## Where do integer related vulnerabilities come from?
...
- New truncations (that exclusively happen on 64-bit systems), e.g. when using atol

- New signedness issues, for example using a signed variable of type int to specify the amount of memory to copy with memcpy.

- Dormant integer overflows - e.g loops using variables of size_t that increment or decrement an (unsigned) int in their loop body.

- Dormant signedness issues, e.g. calls to strlen that do not take a string literal, and assign the return value to an (unsigned) int.

- Unexpected behaviour of library functions, e.g. calls to snprintf that make use of the return value.
