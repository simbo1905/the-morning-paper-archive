---
blog_url: https://blog.acolyer.org/2018/02/09/omniledger-a-secure-scale-out-decentralized-ledger-via-sharding/
blog_title: OmniLedger: a secure, scale-out decentralized ledger via&nbsp;sharding
---
...
- Full decentralisation, with no trusted third parties and no single points of failure

- Shard robustness (OmniLedger partitions state into multiple shards processing in parallel) — each shard must correctly and continuously process transactions assigned to it.

- Secure atomic transactions, but within and across shards

- Scale-out performance with throughput that increases linearly in the number of participating validators

- Low storage overhead so that validators do not need to store the full transaction history
...
## Post navigation
 ...
 &lsaquo; PreviousChainspace: a sharded smart contracts platformNext &rsaquo;SoK: Consensus in the age of blockchains
...
## 
 5 thoughts on &ldquo;OmniLedger: a secure, scale-out decentralized ledger via sharding&rdquo; 
...
- 
 ...
 Pingback: ICO Review: Emotiq - Quantalysus 
 
 
- 
 
 Pingback: RapidChain: scaling blockchain via full sharding | the morning paper 
 
 
- 
 
 Pingback: RapidChain: scaling blockchain via full sharding | Daily AI 
 
 
- 
 
 Pingback: RapidChain: scaling blockchain via full sharding - Official Blockchain News & Information 
