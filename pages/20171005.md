---
blog_url: https://blog.acolyer.org/2017/10/05/hottsql-proving-query-rewrites-with-univalent-sql-semantics/
blog_title: HoTTSQL: Proving query rewrites with univalent SQL&nbsp;semantics
---
...
When you think about it, there’s something special about the number zero when it comes to addition. It acts like an identity value, since and . For multiplication, the number one plays the same role: and . There’s another thing that addition and multiplication have in common, they’re both associative: . This combination of a set and an associative binary option defined over set members with a special identity value is sometimes useful to talk about in the abstract, so we give it a fancy name: a monoid. If we know that something is a monoid, then we already know that the following simple rewrite is valid:
...
We could describe such a semi-ring by a structure . If we can find a way to express SQL semantics in terms of such a structure, then we can use all of the above rules to help us prove query rewrite equivalence. K-relations let us do just that. A K-relation is a function defined over some relation, written , that maps a tuple with given attribute values to a count of the number of times that tuple appears in the relation.
...
- For a predicate b, is 1 is the predicate holds on the tuple, and 0 otherwise.

- For an attribute a, the projection returns the attribute a of the tuple t (not sure I like using the same bracket notation for all three purposes, but there you go...) 

- We’ll also use a squash function, such that is 0 when x is 0, and 1 otherwise. 

- And for equality, has value 0 when x is not equal to y, and 1 otherwise
...
In K-relations as originally defined, the K-relation function needs ‘finite support.’ This means that the set of tuples with non-zero counts must be finite (relations don’t have infinite capacity): is finite.
...
## Post navigation
 ...
 &lsaquo; PreviousActiveClean: Interactive data cleaning for statistical modelingNext &rsaquo;A century of science: globalization of scientific collaborations, citations, and innovations
