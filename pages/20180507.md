---
blog_url: https://blog.acolyer.org/2018/05/07/equality-of-opportunity-in-supervised-learning/
blog_title: Equality of opportunity in supervised&nbsp;learning
---
...
Let X be the set of available features as input to a model, and A be the protected attribute (e.g., race in the example above). Y is the true outcome (i.e., the labels in our labeled dataset), is the predictor which predicts a value of Y given X. We’ll start with the case where Y is a binary decision and A is a binary attribute.

> 

 The notion we propose is "oblivious," in that it is based only on the joint distribution, or joint statistics, of the true target Y, the predictions , and the protected attribute A. In particular, it does not evaluate the features in X nor the functional form of the predictor nor how it was derived.

A predictor satisifies equalized odds with respect to a protected attribute A and outcome Y if and A are independent conditional on Y. Which is equivalent to saying:

For the outcome y=1, will have equal true positive rates across the two demographics A=0 and A=1, and for y=0 it will have equal false positive rates.
...
## Post navigation
 ...
 &lsaquo; PreviousPerformance analysis of cloud applicationsNext &rsaquo;Image-to-image translation with conditional adversarial networks
...
## 
 8 thoughts on &ldquo;Equality of opportunity in supervised learning&rdquo; 
...
I am seeing ”formula doesn't parse” rendered at the end of the sentence starting ”One nice characteristic of these models ”
...
Summary: Fair fair fairness fair? Fairness fairness "fair" fair unfair fairness fairness fairness
...
From Figure 8, it lookalike the model is unfair based on a key implicit assumption which may not be true. The assumption is that given a credit score, a black or a hispanic person is less likely to default than a white or asian. Am I missing something?
...
- 
 ...
 Pingback: Delayed impact of fair machine learning | the morning paper 
...
- 
 ...
 Pingback: We Want Fair AI Algorithms – But How To Define Fairness? (Fairness Series Part 3) - Mostly AI - The Blog 
 
 
- 
 
 Pingback: Diving Deep Into Fair Synthetic Data Generation (Fairness Series Part 5) - Mostly AI - The Blog 
