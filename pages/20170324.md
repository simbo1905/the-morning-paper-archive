---
blog_url: https://blog.acolyer.org/2017/03/24/a-miscellany-of-fun-deep-learning-papers/
blog_title: A miscellany of fun deep learning&nbsp;papers
---
...
- WaveNet speaking in US English

- WaveNet trained without the accompanying text sequence, so that it has to make up what to say:

- WaveNet trained on classical piano music (just the audio, no score!) and left to its own devices to generate whatever it wants to:
...
The architecture of WaveNet is inspired by PixelRNN (See "RNN models for image generation" from a couple of weeks ago). The foundation is very simple - take a waveform with with T values, . And let the probability of the be conditioned on all of the values that precede it: . Now the joint probability of the overall waveform is modelled by:
...
## Post navigation
 ...
 &lsaquo; PreviousRecurrent Neural Network modelsNext &rsaquo;Ethically aligned design
...
## 
 2 thoughts on &ldquo;A miscellany of fun deep learning papers&rdquo; 
...
The papers here are very interesting. Adrian, I must say I am astounded by these daily reviews. Just found this site and subscribed.
...
In "Colorful image colorization", figure 2, they mention that they haven't used 'pool' layers but 'spatial down/up-sampling', which is exactly the same... I'm not sure why is this even relevant; perhaps I'm missing something.
