---
blog_url: https://blog.acolyer.org/2016/01/11/transaction-management-in-r/
blog_title: Transaction Management in the R* Distributed Database Management System &#8211; Part&nbsp;I
---
...
- Enable 'forgetting' of outcomes from commit processing after a short period of time

- Have minimal overhead in terms of log writes and message traffic

- Optimise performance in the no-failure case

- Exploit completely or partially read-only transactions

- Maximize the ability to perform unilateral aborts
...
- The coordinator sends PREPARE messages in parallel to each of the subordinates

- The coordinator waits until either at least one NO VOTE has been received from a subordinate, or all subordinates have returned a YES VOTE. The fulfilment of either of these conditions begins the commit phase.
...
- If instead the subordinate wants to abort the transaction then
...
- Force-writes an abort record

- Sends an ACK message to the coordinator

- Aborts the transaction and 'forgets' it
...
## Post navigation
 ...
 &lsaquo; PreviousARIES: A Transaction Recovery Method Supporting Fine-Granularity Locking and Partial RollbacksNext &rsaquo;Transaction Management in the R* Distributed Database Management System - Part II
...
## 
 8 thoughts on &ldquo;Transaction Management in the R* Distributed Database Management System - Part I&rdquo; 
...
Of course, you knew I'd love this. It brings back many fond memories. I'd love to comment on the 'in-doubt' resolution schemes we adopted, and the reason why the 'telephone' is an inadequate resolution instrument in most cases. It is also worth pointing out that the protocol's sensible handling of 'presumed abort' has the consequence that aborted transactions are undetectable: it is not possible to distinguish, in general, between a transaction that aborted and one that never wert.
...
- 
 ...
 Pingback: Transaction Management in the R* Distributed Database Management System - Part II | the morning paper 
...
Two more typographical slips (my brackets): 

"If at least [on] NO VOTE was received" should be "If at least [one] NO VOTE was received".

"If a subordinate notices the failure of the coordinator before [the ]it has sent its YES VOTE" should be "If a subordinate notices the failure of the coordinator before []it has sent its YES VOTE".

And another comment: it is important that the 'tree' in the 'Hierarchical Two-phase Commit' section *is* a tree. If it is a DAG, intentionally or not, then the protocol can stall, or else continually abort at the commit point. For us this was dubbed the "diamond problem".
...
Thanks Zteve. I'll fix the typos when I get to my desk later on today. Ever thought about writing up some of the lessons learned from those CICS days? I'm sure lots of people would be interested to read that!
