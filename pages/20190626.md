---
blog_url: https://blog.acolyer.org/2019/06/26/designing-far-memory-data-structures/
blog_title: Designing far memory data structures: think outside the&nbsp;box
---
...
- far data in far memory, containing the core content of the data structure
- data caches at clients
...
Counters can be implemented with load, store, and atomic operations using immediate addressing, vectors can use indirect addressing to index into a vector given a base pointer. Clients can subscribe to ranges of addresses to receive notifications when they are modified (notify0) or reach arbitrary values (notifye). Mutexes and barriers are implemented similarly to their shared memory analogues (e.g. compare-and-swap against 0 for a mutex, with a notifye on value 0 if the compare fails).

## Maps

Maps are usually implemented with hash tables or trees, but these don’t work well with far memory. Hash tables have collisions and resizing is disruptive when they are large; tree traversals take O(log n) far accesses.
...
Client-side caching of vectors can generate a lot of notifications when the vector changes. A refreshable vector can return stale data, but include a refresh operation to guarantee freshness of the next lookup. A dynamic policy shifts from client-initiated version checks when data is changing frequently to a notification-based scheme as the update rate slows.
...
I’m a bit troubled by the single sentence at the end of section 4 that is offered without further discussion on its implications: "Because we want notifications to be scalable, they may be delivered in a best-effort fashion." For example, does a client waiting on a mutex potentially stay blocked forever if its notification never arrives? Is it ok for a local cache to potentially go stale beyond some freshness window? This feels like the chink in the armour where a lot of distributed systems problems might come creeping in!
...
## Post navigation
 ...
 &lsaquo; PreviousFast key-value stores: an idea whose time has come and goneNext &rsaquo;Machine learning systems are stuck in a rut
...
## 
 14 thoughts on &ldquo;Designing far memory data structures: think outside the box&rdquo; 
...
I could actually see this become really strong with NVRAM.
...
Is it literature overview without reproducible results or you are restricted by NDAs?
...
- 
 ...
 Pingback: New top story on Hacker News: Designing far memory data structures: think outside the box - News about world 
 
 
- 
 
 Pingback: New top story on Hacker News: Designing far memory data structures: think outside the box - Outside The Know 
 
 
- 
 
 Pingback: New top story on Hacker News: Designing far memory data structures: think outside the box - World Best News 
 
 
- 
 
 Pingback: New top story on Hacker News: Designing far memory data structures: think outside the box - protipsss 
 
 
- 
 
 Pingback: New top story on Hacker News: Designing far memory data structures: think outside the box - Golden News 
 
 
- 
 
 Pingback: New top story on Hacker News: Designing far memory data structures: think outside the box - Latest news 
 
 
- 
 
 Pingback: New top story on Hacker News: Designing far memory data structures: think outside the box - Hckr News 
 
 
- 
 
 Pingback: Designing far memory data structures: think outside the box - INDIA NEWS 
 
 
- 
 
 Pingback: Designing far memory data structures: think outside the box - Boston Area Casino News 
 
 
- 
 
 Pingback: Designing far memory data structures: think outside the box - Hacker News Robot 
 
 
- 
 
 Pingback: Designing far memory data structures: think outside the box - Lapcity 
