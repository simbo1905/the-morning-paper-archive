---
blog_url: https://blog.acolyer.org/2020/10/02/toward-an-api-for-the-real-numbers/
blog_title: Toward an API for the real&nbsp;numbers
---
...
- Expected mathematical properties may not be met. For example, addition is no longer associative: can be very different to ; and starting from zero and adding 0.1 one hundred times may not equal 10.0.

- Results don’t display as users expect, for example using IEEE double precision and Java output formatting rules, , and not the expected 0.8.
...
In IEEE floating point, a real number is represented as a rational approximation of the form for mantissa and exponent . To get back to floating point numbers that behave like reals we need a different representation. The foundation for this is recursive real arithmetic.

> 

A real number is represented as a computable function mapping an error tolerance to a rational number , such that the difference between and is less than the error tolerance . This can be viewed as an implementation of the real numbers from the constructive real analysis.
...
The real number represented by these fields is the arithmetic product of ratFactor and rrFactor. There are rrPropertytags to indicate that rrFactor is exactly 1, or , and parameterised tags in arg to indicate that rrFactor is exactly , , , , , , , or . Finally there is a tag value to indicate that rrFactor is irrational.
...
- If both arguments are known zero, they’re equal

- If both arguments have the same recursive real factor, compare the sign of the factor and the rational factors

- If both arguments have the same rational factors and the same kind of property, compare the property arguments

- If both numbers are square roots of rational numbers, compare the squares

- If none of the above apply, multiply the rational and recursive real factors as recursive reals, and compare increasingly precise approximations until they differ
...
## Post navigation
 ...
 &lsaquo; PreviousSpying on the floating point behavior of existing, unmodified scientific applicationsNext &rsaquo;Understanding operational 5G: a first measurement study on its coverage, performance and energy consumption
