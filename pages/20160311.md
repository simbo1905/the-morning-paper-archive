---
blog_url: https://blog.acolyer.org/2016/03/11/strategic-dialogue-management-via-deep-reinforcement-learning/
blog_title: Strategic Dialogue Management via Deep Reinforcement&nbsp;Learning
---
...
- A stochastic state transition function T :: S &rarr; A &rarr; S that specifies the next state given the current state and action

- A reward function R :: S &rarr; A &rarr; S &rarr; &real; that specifies the reward given to the agent for choosing action a when the environment makes a transition from state s to state s'. 

- A policy function &pi; :: S &rarr; A that defines a mapping from states to actions.

The goal of the agent is to maximise its cumulative discounted reward, by convention represented as the function Q* :: S &rarr; A &rarr; &real;, the sum of rewards at each time step, discounted by factor &gamma; at each time step.

In Deep Reinforcement Learning (DRL) we induce a Q :: S &rarr; A &rarr; &Theta;i &rarr; &real; function that approximates Q* using a multilayer convolutional neural network, where &Theta;iare the parameters (weights) of the neural network at iteration i.
...
## Post navigation
 ...
 &lsaquo; PreviousMemory NetworksNext &rsaquo;Secrets, Lies, and Account Recovery: Lessons from the Use of Personal Knowledge Questions at Google
...
## 
 One thought on &ldquo;Strategic Dialogue Management via Deep Reinforcement Learning&rdquo; 
...
- 
 ...
 Pingback: Four short links: 11 March 2016 - O'Reilly Radar 
