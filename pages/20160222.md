---
blog_url: https://blog.acolyer.org/2016/02/22/includeos/
blog_title: IncludeOS: A minimal, resource efficient unikernel for cloud&nbsp;systems
---
...
## Post navigation
 ...
 &lsaquo; PreviousFormal Requirements for Virtualizable Third Generation ArchitecturesNext &rsaquo;Not-quite-so-broken TLS: lessons in re-engineering a security protocol specification and implementation
...
## 
 2 thoughts on &ldquo;IncludeOS: A minimal, resource efficient unikernel for cloud systems&rdquo; 
...
An update from Alfred Bratterud on the most recent IncludeOS developments since the paper was published:

The most important changes since the paper:

* We switched from EASTL to LLVM's libc++, so we now have a complete C++ standard library

 - Not all features have backend implementation, e.g. C++ thread semantics won't work as we're (at least for now) trying to avoid classical threads.

 - We do have a read-only FAT file system now (dev branch), but it's not 100% clear how or if we can get it to work with C++ fstreams, which depend on synchronous POSIX calls.

 - Interesting note: EA Games recently (Feb. 2016) made an official github repo (https://github.com/electronicarts/EASTL) with what looks like an active and official release of EASTL. We'll be looking at that with great interest.

* To support libc++ we also implemented exceptions

 - Your C++ environment should now be very much like everywhere else, except for interaction with the underlying OS. E.g. - no fork, exec or threads, and no blocking file system calls.

* The switch to libc++ / exceptions made the images significantly bigger. Our current footprint starts at around ~700Kb. We might do an absolute minimal version again, with EASTL, but as 32 bit x86 VM's always have at least 1 MB of memory it might not be that much to save in actual memory footprint.

* We switched from GCC to LLVM/Clang-based toolchain

 - This was in order to ease porting of libc++, and I have to say we've never once looked back.

* We do have working TCP in the dev branch (just enough to serve a static http-page in the master branch). Still lots to be done on the testing side, but we're … Really happy with the design so far.
