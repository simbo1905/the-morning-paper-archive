---
blog_url: https://blog.acolyer.org/2016/03/10/memory-networks/
blog_title: Memory Networks
---
...
> 
 ...
 More sophisticated variants of G could go back and update earlier stored memories (potentially, all memories) based on the new evidence from the current input x. If the input is at the character or word level one could group inputs (i.e., by segmenting them into chunks) and store each chunk in a memory slot.
...
## Post navigation
 ...
 &lsaquo; PreviousNeural Turing MachinesNext &rsaquo;Strategic Dialogue Management via Deep Reinforcement Learning
...
## 
 14 thoughts on &ldquo;Memory Networks&rdquo; 
...
- 
 ...
 Pingback: Big Analytics Roundup (March 14, 2016) | The Big Analytics Blog 
 
 
- 
 
 Pingback: A survey of available corpora for building data-driven dialogue systems | the morning paper 
 
 
- 
 
 Pingback: Machine learning for dialog state tracking: a review | the morning paper 
...
- 
 ...
 Pingback: Looking beyond Automata Models: Transducing and Grammar Learning with Neural Machines - Automaton Learning 
...
That was a great Article, really helped me in clearing many cob-webs.

I had a question though, In the 4 stages which are mentioned above , What exactly do we do in Stage 'I', do we do word2vec conversion for all the words separately and store them. or we do a word2vec for the full sentence.. or we don't need to do word2vec at all in this case ?
...
Hi Nitin, FB are really describing a general architecture here, so in processing the *I*nput any conversion into a feature representation suitable for the problem at hand can be used. Whatever that representation is (which could indeed be e.g. word or sentence based vectors for language processing) it gets used as a kind of index into the memory...
...
Adrian, also, I was going through Papers by Jason Wetson on Memory Networks. Do you have any other exhaustive source, where we can find a descriptive implementation of QA using MemNN.
...
I don't maintain my own list, but this resource is pretty good: https://github.com/songrotek/Deep-Learning-Papers-Reading-Roadmap/blob/master/README.md, and section 2.5 is related to NTM / MemNets etc..
...
- 
 ...
 Pingback: Recurrent Neural Network models | the morning paper 
 
 
- 
 
 Pingback: Deep Learning in Neural Networks: An Overview | the morning paper 
...
> For k > 1, the n_th memory (1 < n ≤ k) is found by comparing both the original input sentence and all of the n-1 supporting memories found so far against the sentences in memory. 

Is that mean one may have duplicate retrieved memories ?
