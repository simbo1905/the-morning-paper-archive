---
blog_url: https://blog.acolyer.org/2019/03/08/a-generalised-solution-to-distributed-consensus/
blog_title: A generalised solution to distributed&nbsp;consensus
---
...
We have a fixed set of n servers, . Each server has a tape it can write to, aka, an infinite array of write-once persistent registers, .
...
- Quorum agreement. A client may only output a (non-nil) value if it has read from a quorum of servers in the same register set. 
- New value. A client may only write a (non-nil) value provided that is the client’s input value or that the client has read from a register.
- Current decision. A client may only write a (non-nil) value to register on server provided that if is decided in register set by a quorum where then no value where can also be decided in register set . 
- Previous decisions. A client may only write a (non-nil) value to register provided no value where can be decided by the quorums in registers sets 0 to . 
...
Obeying these rules places some further constraints on the system configuration. Say we have two quorums for register set i, and . A client writing to the register on has no way of knowing whether or not some other client might be writing a different value to , hence we couldn’t uphold rule 3. There are three ways we can handle this situation:
...
- MAYBE - if this quorum reaches a decision, then the value will be 
- DECIDED - the value has been decided by the quorum (a final state)
...
- If the client reads a non-nil value for register set and all quorum members have the same value then the state of the quorum is set to DECIDED. Otherwise the state is set to MAYBE . In this latter case, for the same quorum in earlier register sets a value of ANY will also be set to MAYBE , but a value of MAYBE where will be updated to NONE. 
...
## Post navigation
 ...
 &lsaquo; PreviousKeeping CALM: when distributed consistency is easyNext &rsaquo;Efficient synchronisation of state-based CRDTs
...
## 
 12 thoughts on &ldquo;A generalised solution to distributed consensus&rdquo; 
...
- 
 ...
 Pingback: New top story on Hacker News: A generalised solution to distributed consensus - Latest news 
 
 
- 
 
 Pingback: New top story on Hacker News: A generalised solution to distributed consensus - Hckr News 
 
 
- 
 
 Pingback: New top story on Hacker News: A generalised solution to distributed consensus - World Best News 
 
 
- 
 
 Pingback: New top story on Hacker News: A generalised solution to distributed consensus - Golden News 
 
 
- 
 
 Pingback: New top story on Hacker News: A generalised solution to distributed consensus - Outside The Know 
 
 
- 
 
 Pingback: A generalised solution to distributed consensus | My Tech Blog 
 
 
- 
 
 Pingback: New top story on Hacker News: A generalised solution to distributed consensus - News about world 
 
 
- 
 
 Pingback: A generalised solution to distributed consensus - Hacker News Robot 
 
 
- 
 
 Pingback: A generalised solution to distributed consensus - TARIAN LIMITED 
...
- 
 ...
 Pingback: A generalised solution to distributed consensus 
...
I acquire a deep knowledge about blockchain consensus algorithms from your blog. Your defined consensus algorithm categories let me to know more about the blockchain technology. I too like to suggest a consensus algorithms blockchaain development company here

https://www.osiztechnologies.com/blockchain-development
