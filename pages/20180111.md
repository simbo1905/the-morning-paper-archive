---
blog_url: https://blog.acolyer.org/2018/01/11/emergent-complexity-via-multi-agent-competition/
blog_title: Emergent complexity via multi-agent&nbsp;competition
---
...
The fix for this is to train against randomly selected older versions of the opponent rather than simply always using the most recent. The following tables show the results in Sumo training for the humanoid and ant agents ($\delta = 1.0$ corresponds to the most recent agent, and corresponds to uniform sampling over the whole history).

Sampling over the whole history works best for ants, but for humanoids gives the best win rate.
...
## Post navigation
 ...
 &lsaquo; PreviousMastering chess and shogi by self-play with a general reinforcement learning algorithmNext &rsaquo;One model to learn them all
