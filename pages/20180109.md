---
blog_url: https://blog.acolyer.org/2018/01/09/the-case-for-learned-index-structures-part-ii/
blog_title: The case for learned index structures &#8211; Part&nbsp;II
---
...
Based on this insight, the authors develop a new biased quaternary search strategy. In a quaternary search, instead of picking one new middle point to to test as in binary search, three new test points are chosen (thus dividing the dataset into four quarters, as opposed to two halves). The three initial middle points for the quaternary search are set to be , , and , where is the predicted position. “That is we make a guess that most of our predictions are accurate and focus our attention first around the position estimate and then we continue with traditional quaternary search.”
...
> 
 ...
 Surprisingly, learning the CDF of the key distribution is one potential way to learn a better hash function... we can scale the CDF by the targeted size M of the hash-map and us , with key K as our hash-function. If the model F perfectly learned the CDF, no conflicts would exist.
...
From an ML perspective, we can think of a Bloom-filter as a classifier, which outputs the probability that a given key is in the database. We can set the probability threshold for considering a key to be in the set such that we can achieve any given target false positive rate. Unlike Bloom filters though, we can’t guarantee the absence of false negatives. The solution to this problem is neat: take the set of false negatives produced by the learned function , and create a Bloom filter just for this subset of keys. Now on a lookup if says the key is present with probability greater than we can respond in the affirmative. If the probability is less than this threshold, a second check to the false negatives Bloom filter is added before returning the final decision.
...
## Post navigation
 ...
 &lsaquo; PreviousThe case for learned index structures - part INext &rsaquo;Mastering chess and shogi by self-play with a general reinforcement learning algorithm
...
## 
 9 thoughts on &ldquo;The case for learned index structures - Part II&rdquo; 
...
More work along this line exists in the Database community. Specifically in the context of Approximate Query Processing (AQP) in which approximate results are obtained instead of the actual answers to queries. In "Learning to accurately COUNT with query-driven predictive analytics" the authors construct ML models to predict the results for range queries instead of using data-structures (histograms, wavelets, self-tuning histograms, samples) to approximate the answers.
...
- 
 ...
 Pingback: Michael Tsai - Blog - The Case for Learned Index Structures 
...
"The more accurate the model, the better the savings in Bloom filter size."

It seems like the graph and numbers (1% false positive: 46% savings / 0.01% false positive : 27% savings) gives the opposite correlation.
...
- 
 ...
 Pingback: SageDB: a learned database system | the morning paper 
...
- 
 ...
 Pingback: Meta-learning neural Bloom filters - Newbyo 
 
 
- 
 
 Pingback: The case for learned index structures – Part II 1/9/18 - Data Systems Group 
