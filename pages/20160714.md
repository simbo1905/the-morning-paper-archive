---
blog_url: https://blog.acolyer.org/2016/07/14/dbsherlock-a-performance-diagnostic-tool-for-transactional-databases/
blog_title: DBSherlock: A performance diagnostic tool for transactional&nbsp;databases
---
...
- If the nearest adjacent partitions have different types, then the partition is assigned the label of the closer partition. The distance calculation includes an anomaly distance multiplier factor &delta; by which the distance to an abnormal adjacent neighbour is multiplied before doing the comparison. If &delta; > 1 of course, this favours marking a partition as normal. DBSherlock uses &delta; = 10 by default.
...
- After normalizing the attribute values so that they all fall on a spectrum from 0..1, the average value of the attribute in the normal range and the average value of the attribute in the abnormal range are separated by at least &theta;, where &theta; is a configurable normalized difference threshold.
...
Categorical attribute predicates are simpler, and DBSherlock simply generates the predicate Attr &isin; {p | p.label = abnormal}.
...
The secondary symptom predicates can be pruned with a small amount of domain knowledge. This knowledge is expressed as a set of rules of the form Attri &rarr; Attrj with the meaning that if predicates for both Attri and Attrj are generated, then the predicate for Attrj is likely to be a secondary symptom. Cycles are not allowed in the rule set.

As an example, for MySQL on Linux, there are just four rules:

- DBMS CPU Usage &rarr; OS CPU Usage. (if DBMS CPU Usage is high, we probably don't need to also flag OS CPU Usage)

- OS Allocated Pages &rarr; OS Free Pages; OS Used Swap Space &rarr; OS Free Swap Space; OS CPU Usage &rarr; OS CPU Idle. (For these three, one attribute is always completely determined by the other, and so is uninteresting).
...
## Related
 ...
 DatastoresOperationsPerformance 
...
## Post navigation
 ...
 &lsaquo; PreviousAmbry: LinkedIn's scalable geo-distributed object storeNext &rsaquo;Spheres of influence for more effective viral marketing
...
## 
 5 thoughts on &ldquo;DBSherlock: A performance diagnostic tool for transactional databases&rdquo; 
...
- 
 ...
 Pingback: Not quite that elementary, dear Watson. DBSherlock | The Information Age 
