---
blog_url: https://blog.acolyer.org/2017/12/07/be-selfish-and-avoid-dilemmas-fork-after-withholding-attacks-on-bitcoin/
blog_title: Be selfish and avoid dilemmas: fork-after-withholding attacks on&nbsp;Bitcoin
---
...
- It’s more lucrative than a block withholding attack (about 56% more reward for the attacker)

- When pools attack each other, under reasonable assumptions for the computational power and network capabilities, the miner’s dilemma does not hold - the larger pool takes the extra reward, and the smaller pool takes a loss. 

- There doesn’t seem to be a viable defence.
...
The size of the reward depends on (amongst other things, such as relative computing power), a variable , the probability that an attacker’s FPoW found through infiltration mining and subsequently released will be selected as the main chain. The higher this probability, the greater the reward.

> 

 An attacker can earn an extra reward regardless of or the target pool size . Therefore, an attacker should always run the FAW attack to increase her own reward... Conversely, a target pool always suffers a loss in the presence of an attacker.

When two pools attack each other, playing the FAW attack game then:

- The miner’s dilemma no longer applies

- The game outcome is based on pool size, and the larger pool wins the game.

Consider two pools, we could call them Pool1 and Pool2. Let Pool1 control a fraction of the overall mining power, and Pool2 control a fraction of the overall mining power . The plot below examines what happens in an FAW attack game between the two pools, for varying values of .

> 

 When is 1, the borderline is exactly the line . In other words, the larger pool always earns the extra reward, and the smaller pool takes a loss. Therefore, the result becomes dependent on pool size, even in the region where the miner’s dilemma holds in the BWH attack game. Furthermore, the region in which the miner’s dilemma does not hold exists even if is less than 1. In summary, under reasonable conditions for two pool’s computational power and network capabilities, the largest pool earns the extra reward. This makes the FAW attack a dominant strategy for any large pool to launch against smaller pools.
...
## Post navigation
 ...
 &lsaquo; PreviousThe dynamics of innocent flesh on the bone: code reuse ten years laterNext &rsaquo;Economic factors of vulnerability trade and exploitation
...
## 
 7 thoughts on &ldquo;Be selfish and avoid dilemmas: fork-after-withholding attacks on Bitcoin&rdquo; 
...
- 
 ...
 Pingback: Be selfish and avoid dilemmas: fork-after-withholding attacks on Bitcoin | Blockchain Wiki 
 
 
- 
 
 Pingback: Bitcoin News Digest Thursday, December 7, 2017 - zentrade.online 
 
 
- 
 
 Pingback: Bitcoin is not trustless &bull; Cryptocurrency decentral market headlines 
 
 
- 
 
 Pingback: Why Bitcoin is not trustless - Cryptofundamentals | Cryptofundamentals 
...
- 
 ...
 Pingback: SoK: Research perspective and challenges for Bitcoin and cryptocurrency - Part I | the morning paper 
 
 
- 
 
 Pingback: A survey on security and privacy issues of Bitcoin | the morning paper 
