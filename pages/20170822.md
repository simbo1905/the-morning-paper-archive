---
blog_url: https://blog.acolyer.org/2017/08/22/javascript-for-extending-low-latency-in-memory-key-value-stores/
blog_title: JavaScript for extending low-latency in-memory key-value&nbsp;stores
---
...
- Applications that chase inter-record dependencies (fetching one record, then using the returned value to fetch other records) need multiple round trips, and even the 5µs round trip time RAMCloud offers is too high under these conditions. In fact, 5µs turns out to be a very awkward amount of time: too long for spinning without killing throughput, and too short to context switch without losing most of the gains in switching overhead.
...
- Near native performance. RAMCloud keeps all data in DRAM and uses kernel bypass networking with operations dispatched in 1.9µs. Low latency is what it's all about, sacrificing that makes no sense in a RAMCloud world.
...
## Related
 ...
 DatastoresProgramming Languages 
...
## Post navigation
 ...
 &lsaquo; PreviousParacloud: bringing application insight into cloud operationsNext &rsaquo;Growing a protocol
...
## 
 10 thoughts on &ldquo;JavaScript for extending low-latency in-memory key-value stores&rdquo; 
...
- 
 ...
 Pingback: JavaScript for extending low-latency in-memory key-value stores - Full-Stack Feed 
...
- 
 ...
 Pingback: Bringing the web up to speed with WebAssembly | the morning paper 
...
- 
 ...
 Pingback: Bringing the win as much as breeze with WebAssembly | A1A 
