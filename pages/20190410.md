---
blog_url: https://blog.acolyer.org/2019/04/10/dont-trust-the-locals-investigating-the-prevalence-of-persistent-client-side-cross-site-scripting-in-the-wild/
blog_title: Don’t trust the locals: investigating the prevalence of persistent client-side cross-site scripting in the&nbsp;wild
---
...
## Post navigation
 ...
 &lsaquo; PreviousHow bad can it git? Characterizing secret leakage in public GitHub repositoriesNext &rsaquo;Master of web puppets: abusing web browsers for persistent and stealthy computation
...
## 
 21 thoughts on &ldquo;Don’t trust the locals: investigating the prevalence of persistent client-side cross-site scripting in the wild&rdquo; 
...
- 
 ...
 Pingback: Don’t trust the locals: investigating the prevalence of persistent client-side cross-site scripting in the wild | the morning paper 
 
 
- 
 
 Pingback: New top story on Hacker News: Don’t trust the locals: investigating prevalence of persistent client-side XSS - Hckr News 
 
 
- 
 
 Pingback: New top story on Hacker News: Don’t trust the locals: investigating prevalence of persistent client-side XSS - Golden News 
 
 
- 
 
 Pingback: New top story on Hacker News: Don’t trust the locals: investigating prevalence of persistent client-side XSS - Latest news 
 
 
- 
 
 Pingback: New top story on Hacker News: Don’t trust the locals: investigating prevalence of persistent client-side XSS - World Best News 
 
 
- 
 
 Pingback: New top story on Hacker News: Don’t trust the locals: investigating prevalence of persistent client-side XSS - News about world 
 
 
- 
 
 Pingback: New top story on Hacker News: Don’t trust the locals: investigating prevalence of persistent client-side XSS - Outside The Know 
 
 
- 
 
 Pingback: Don’t trust the locals: investigating prevalence of persistent client-side XSS - INDIA NEWS 
 
 
- 
 
 Pingback: Don’t trust the locals: investigating the prevalence of persistent client-side cross-site scripting in the wild - JNW 
 
 
- 
 
 Pingback: Don’t trust the locals: investigating prevalence of persistent client-side XSS | My Tech Blog 
 
 
- 
 
 Pingback: Collective #507 - Unsorted 
 
 
- 
 
 Pingback: Don’t trust the locals: investigating prevalence of persistent client-side XSS - Hacker News Robot 
 
 
- 
 
 Pingback: Collective #507 - Find Right Software 
 
 
- 
 
 Pingback: Collective #507 - News 20h 
 
 
- 
 
 Pingback: The Prevalence of Persistent XSS - Curated SQL 
 
 
- 
 
 Pingback: Inspirational Website of the Week: | Digitallife33 
 
 
- 
 
 Pingback: Дайджест свежих материалов из мира фронтенда за последнюю неделю №360 (7 — 14 апреля 2019) -- MAILSGUN.RU 
 
 
- 
 
 Pingback: Bug Bytes #14 - Better Exfiltration via HTML Injection by @fransrosen, Dell KACE K1000 RCE by @MrTuxracer & BurpFeed - INTIGRITI 
...
Please forgive me if this comment posts twice - had to reset my WordPress login.

I’m a bit confused about the part discussing injecting the malicious code, specifically: “There are two main routes an attacker can use to persist malicious payloads: an in-network attacker can hijack connections over HTTP, or the attacker can lure the victim to visit a website under the attacker’s control” If the attacker gets you to leave good.com and go to bad.com, any code that writes to LocalStorage would only apply to bad.com. So how does luring a user away impact LocalStorage on the original site?
...
there is a minor type here: 1 to many T in 

.. HTTTP Strict Transport ...
...
The potential for attacks like this against localStorage seems overstated to me. Since localStorage for HTTP and HTTPS versions of a site are entirely separate, you can't set a malicious payload for a HTTPS-only site by MITMing an attempt to connect to it over HTTP. That leaves the potential of setting a malicious payload using an XSS vulnerability. Okay, sure, but having an XSS vulnerability in the first place is a big deal, and all that such an attack permits is making the vulnerability persistent. Not really that big or interesting a step up from just having an ordinary XSS vulnerability in the first place.

The idea of using a MITM attack to set a cookie is more interesting, but it's curious that the authors don't explore the most obvious application of this: simply overwriting the user's authentication cookie so that they become logged in as a user you can also access and view the actions of. No need for any vulnerable JavaScript to take advantage of that attack vector.
