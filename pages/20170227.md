---
blog_url: https://blog.acolyer.org/2017/02/27/understanding-generalisation-and-transfer-learning-in-deep-neural-networks/
blog_title: Understanding, generalisation, and transfer learning in deep neural&nbsp;networks
---
...
## Post navigation
 ...
 &lsaquo; PreviousAn experiment with awesome deep learning papersNext &rsaquo;When DNNs go wrong - adversarial examples and what we can learn from them
...
## 
 9 thoughts on &ldquo;Understanding, generalisation, and transfer learning in deep neural networks&rdquo; 
...
About your conclusions: «The short summary – transfers can improve generalization performance. Two issues impact how well transfer occurs: fragile co-adaptation of middle layers, and specialisation of higher layers.», this is quite not correct. This article was even moked at NIPS, when it has been presented orally.

Turns out that the poor performance of AnB is the direct consequence of the optimisation problem that we try to address. More precisely, if you freeze the base, you'll end up with far fewer degrees of freedom, hence, your optimisation will suffer. Unlock the base, and magically you'll have many more parameters for you to play with. It performs better that BnB+ because of lower overfitting due to a larger input data set. So, it was a unfortunate article...
...
I think the short summary above is right, it does have better perfermance.You say "this is quite not correct", why?
...
The aim in «Distilling the knowledge in a neural network» is noble, but it doesn't work for anything that is not a toy data set. More specifically, MNIST is the only one that showed some decent results...

P.S. It's considered unacceptable to post arXiv links to PDFs instead of their abstract page 😉
...
It's very helpful getting an "insider" perspective on some of these papers, thank you. And I've wondered what the accepted way is to link to arXiv papers is too! (I link to pdf's for consistency across all of the different paper sources that TMP touches). I'm changing that convention for some of the ACM DL papers, so I guess I should swap now for arXiv too :).
...
Great! This is because, from mobile, someone may just want to view the abstract (the PDF gets downloaded immediately, otherwise) or choose a different version (with the PDF link you get the version that has been linked or the most recent one).
...
Anyhow, thank you for writing these! They help refresh some recent historical milestones in the field. I am even considering to use these for my class. These summaries are beautifully written!
...
- 
 ...
 Pingback: Machine Learning Artistic - Susdabble 
...
Thanks for such an informative article. I have a query.....I am running a CNN classifier for 2 classes where positive class is an object and Negative class is everything that is not that object + everything that can occur in background. While creating training data for positive class .......should I keep only Images that have positive class covering more than 80%?

I have around 8500 images for positive class(7000 images have positive class covering more than 80% of Image while 1500 Images have object in 30-40%of Images)

My overall accuracy decreases if I remove these 1500 Images from Training class during training.
