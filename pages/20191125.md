---
blog_url: https://blog.acolyer.org/2019/11/25/mergeable-replicated-data-types-part-i/
blog_title: Mergeable replicated data types &#8211; Part&nbsp;I
---
...
The proposal in this case is that the result of the merge operation be 9. Merging versions and with a lowest common ancestor , the three-way merge function is defined as:
...
Consider a lowest common ancestor state of the queue (e.g. [1;2] in the preceding example), and two concurrent versions and to be merged, what properties should hold between , , and and the resulting merge state ?

- A queue element that was never consumed (i.e. was not popped in either version) should still be present in .

- For every in , if is popped in either or then it should not be present in . (The assumption here is that two entries with the same numeric value are distinct elements.)

- If is not in and it is newly pushed into (i.e., present in) or then it should be in .

- Partial ordering is preserved: for every and in (or , or ) then if and are not deleted then also occurs before in .With these rules in mind we can map queue state to relations using a pair of relations: is a set containing all the members of a queue, and is an occurs-before relation relating every pair of elements and in the queue such that if occurs before in the queue, then .
...
One implementation strategy to materialise the queue state from the pair of relations and is to construct a directed graph with vertices and edges , and ties broken with additional arbitration edges. A topological ordering of the graph then yields the queue.

> 
We have generalized the aforementioned graph-based approach for concretizing ordering relations, and abstracted it away as a library function . Give ord, an arbitration order the function concretizes an ordering relation of a data structure (not necessarily a total order) as a graph isomporphic to that structure, using the abritration order to break ties...

## The general case

We’re getting closer to the generalised MRDT approach now. For each data type (e.g., list and queue as we’ve seen so far), we can define characteristic relations that define the representation in the relational domain (e.g., and for the queue example). The characteristic relations for a range of data types are shown in the table below.
...
## Post navigation
 ...
 &lsaquo; PreviousPlanAlyzer: assessing threats to the validity of online experimentsNext &rsaquo;Mergeable replicated data types - Part II
...
## 
 16 thoughts on &ldquo;Mergeable replicated data types - Part I&rdquo; 
...
- 
 ...
 Pingback: New top story on Hacker News: Mergeable replicated data types – Part I - Latest news 
 
 
- 
 
 Pingback: New top story on Hacker News: Mergeable replicated data types – Part I - Outside The Know 
 
 
- 
 
 Pingback: New top story on Hacker News: Mergeable replicated data types – Part I - protipsss 
 
 
- 
 
 Pingback: New top story on Hacker News: Mergeable replicated data types – Part I - Ultimate News 
 
 
- 
 
 Pingback: New top story on Hacker News: Mergeable replicated data types – Part I - Hckr News 
 
 
- 
 
 Pingback: Mergeable replicated data types – Part I - INDIA NEWS 
 
 
- 
 
 Pingback: New top story on Hacker News: Mergeable replicated data types – Part I - News about world 
 
 
- 
 
 Pingback: New top story on Hacker News: Mergeable replicated data types – Part I - Golden News 
 
 
- 
 
 Pingback: Mergeable replicated data types – Part I - Gadgets and Technology News 
 
 
- 
 
 Pingback: Mergeable replicated data types – Part I - Hacker News Robot 
...
- 
 ...
 Pingback: Mergeable replicated data types - Part II - the morning paper 
