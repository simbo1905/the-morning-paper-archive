---
blog_url: https://blog.acolyer.org/2018/01/23/why-is-random-testing-effective-for-partition-tolerance-bugs/
blog_title: Why is random testing effective for partition tolerance&nbsp;bugs?
---
...
- Suppose a randomly chosen test satisfies the goal with probability .

- Then the probability such a does not satisfy the goal is simply .

- If we take N such independently chosen random tests, then the probability than none of them satisfy the goal is .

- If we have m coverage goals, then the set of N tests above is not a covering family with a probability at most . 

- For a given , if we pick the number of tests N to be then the probability this not a covering family can be made less than .

- And therefore, we have a covering family of tests with probability at least .
...
## Post navigation
 ...
 &lsaquo; PreviousProgramming and proving with distributed protocolsNext &rsaquo;Linear Haskell: Practical linearity in a higher-order polymorphic language
...
## 
 One thought on &ldquo;Why is random testing effective for partition tolerance bugs?&rdquo; 
...
- 
 ...
 Pingback: SRE Weekly Issue #107 - SRE WEEKLY 
