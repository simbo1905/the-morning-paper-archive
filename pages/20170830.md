---
blog_url: https://blog.acolyer.org/2017/08/30/a-concurrent-perspective-on-smart-contracts/
blog_title: A concurrent perspective on smart&nbsp;contracts
---
...
- The order of transactions included in a block is not determined at the moment of transaction execution. Outcomes may depend on transaction ordering. 

- Contract logic may be spread across several blockchain transactions (e.g., when contracts 'communicate' with the world outside of the blockchain).

- A contract that calls another contract exhibits a form of cooperative multi-tasking, a yield. But the called contract may run code unanticipated by the caller (the source of re-entrancy bugs for example). 

- A contract could be used as a service managing access to a shared resource (a bit like a concurrent library). "As multi-contract transactions are becoming more ubiquitous, various interference patterns can be observed and, thus, should be accounted for."
...
## Post navigation
 ...
 &lsaquo; PreviousSeeing is believing: a client-centric specification of database isolationNext &rsaquo;Adding concurrency to smart contracts
...
## 
 6 thoughts on &ldquo;A concurrent perspective on smart contracts&rdquo; 
...
Makes me curious what Pony reference capabilities (https://tutorial.ponylang.org/capabilities/) could contribute to smart contracts.
...
- 
 ...
 Pingback: New top story on Hacker News: A concurrent perspective on smart contracts - The Internet Yard 
 
 
- 
 
 Pingback: Adding concurrency to smart contracts | the morning paper 
