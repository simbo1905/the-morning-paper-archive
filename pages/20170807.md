---
blog_url: https://blog.acolyer.org/2017/08/07/acidrain-concurrency-related-attacks-on-database-backed-web-applications/
blog_title: ACIDRain: concurrency-related attacks on database backed web&nbsp;applications
---
...
As a result of the attack, Flexcoin had to shut down.
...
## Related
 ...
 ConsistencySecurityTransaction processing 
...
## Post navigation
 ...
 &lsaquo; PreviousEnd of term, and Orders of MagnitudeNext &rsaquo;A general purpose counting filter: making every bit count
...
## 
 6 thoughts on &ldquo;ACIDRain: concurrency-related attacks on database backed web applications&rdquo; 
...
Note that although a variety of concurrency-related bugs may exist in an application, here we’re just interest in finding those that impact database state.

-> should be 'interested'.

I'm glad to start reading your articles again!
...
- 
 ...
 Pingback: IT Security Weekend Catch Up – August 11, 2017 - BadCyber 
...
Just use database level constraints and you won't violate those invariants anymore, even with faulty application logic.
...
This reminded me of a talk by Udi Dahan, where he argues that for collaborative domains business logic can be turned into a long-running process that continuously verifies that the data model satisfies the invariants and is also consistent with the outside world (e.g., item in the shopping cart was dropped on the floor and is no longer available). I wonder if this approach could be used to side-step the whole DB consistency issue, albeit at the cost of higher application complexity. See https://www.youtube.com/watch?v=fWU8ZK0Dmxs.
...
- 
 ...
 Pingback: Weekendowa Lektura 2017-08-11 - bierzcie i czytajcie | Zaufana Trzecia Strona 
