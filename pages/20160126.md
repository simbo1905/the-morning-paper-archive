---
blog_url: https://blog.acolyer.org/2016/01/26/arabesque/
blog_title: Arabesque: A System for Distributed Graph&nbsp;Mining
---
...
Arabesque can be set to run in edge-based or vertex-based exploration mode. In the very first iteration, each edge (or vertex respectively) is a candidate embedding, and at each step the system creates candidate embeddings by adding one edge (or vertex, for vertex-based exploration). Each candidate embedding is passed to the user-defined filter function &phi;. If the filter function returns true, then the embedding e' is passed to the user-defined process function &pi;, which may produce some output to be aggregated. The set of filtered embeddings, F, then become the new input for the next step. Computation is terminated when F is empty at the end of a step.
...
These spurious embeddings can be filtered out using the user-provided &phi; (filter) function.
...
## Post navigation
 ...
 &lsaquo; PreviousThe Design and Implementation of the Wave Transactional FilesystemNext &rsaquo;Petuum: A New Platform for Distributed Machine Learning on Big Data
...
## 
 3 thoughts on &ldquo;Arabesque: A System for Distributed Graph Mining&rdquo; 
...
- 
 ...
 Pingback: Big Analytics Roundup (February 1, 2016) | The Big Analytics Blog 
