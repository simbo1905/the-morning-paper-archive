---
blog_url: https://blog.acolyer.org/2019/03/11/efficient-synchronisation-of-state-based-crdts/
blog_title: Efficient synchronisation of state-based&nbsp;CRDTs
---
...
> 
Delta-based CRDTs... define delta-mutators that return a delta ( ), typically much smaller than the full state of the replica, to be merged with the local state. The same is also added to an outbound buffer, to be periodically propagated to remote replicas. Delta-based CRDTs have been adopted in industry as part of the Akka Distributed Data framework and IPFS.
...
To bring all this to life we need a couple of examples. The paper uses grow-only CRDT counters and grow-only sets. State-based CRDTs are based upon a join semi-lattice, , a set of mutators that update that state, and a binary join operator that derives the least upper bound for any two elements of .
...
Let’s look at one particular evolution of a GSet in a scenario with two replicas, A and B. In the diagram below, marks a point in time where synchronisation occurs. For example, at , replica B sends the delta to replica A. A has also received a local update. At , replica A sends all updates it has received since the last time it propagated changes back to B. This is the set . Meanwhile replica B has added , and at it sends all of its changes since back to A.

It’s easy to spot some redundancy in transmission here. At , B sends to A, and at , A is including in the update it sends back to _A_ again. All of the underlined elements in the figure above are redundant transmissions of this type.

So it’s not exactly rocket science...

> ... by simply tracking the origin of each -group in the -buffer, replicas can avoid back-propagation of -groups.
...
Here we see that at , C is sending delta to D. Then C receives an update from A at . At the update is propagated from C to D, with the result that C has redundantly sent to D in this second transmission. The RR optimisation removes this redundant transfer, we should...

> ... remove redundant state in received -groups (RR) before adding them to the delta buffer.
...
Now, those ideas are formalised using the notion of a join-irreducible state. A join irreducible state is one that can’t be reconstructed from a join of some finite set of states not containing . So the set is join irreducible, but the set is not because we can construct it by joining and . Every CRDT has a unique irredundant decomposition of a state into a set of join irreducible states. For the GSet for example, these are just the sets containing individual elements, and for the GCounter they are the individual replica-local counters. We can use this decomposition to find the minimum delta, between two states and .

Here’s the classic delta-based synchronization algorithm for replica , updated to include the BP and RR optimisations. Highlighted lines show the changes.

For BP, each -buffer is tagged with its origin at line 5, so that we can filter out redundant deltas when sending updates in line 11. For RR we extract the minimal delta in line 15.
...
## Post navigation
 ...
 &lsaquo; PreviousA generalised solution to distributed consensusNext &rsaquo;Cloud computing simplified: a Berkeley view on serverless computing
