---
blog_url: https://blog.acolyer.org/2017/11/29/concrete-problems-in-ai-safety/
blog_title: Concrete problems in AI&nbsp;safety
---
...
- Negative side effects - the agent causes unintended and negative consequences in its environment (e.g., knocking over a vase on its way to clean up a mess).

- Reward hacking - the agent might game its reward function (e.g., covering over messes with materials it can’t see through).

- Insufficient oversight - arising from situations where the objective function is too expensive to evaluate frequently (e.g., deciding how to handle stray objects in the environment- we want the object to treat candy wrappers and cell phones differently).

- (Un)safe exploration - the agent makes exploratory moves with bad repercussions (e.g., experimenting with mopping strategies is ok, but putting a mop in an electrical outlet is not). 

- Fragility in the face of distributional shift - what happens when the real world doesn’t perfectly match the training environment? (E.g., strategies learned cleaning factory floors may not be safe in an office context). 
...
## Post navigation
 ...
 &lsaquo; PreviousPopularity predictions of Facebook videos for higher quality streamingNext &rsaquo;The role of software in spacecraft accidents
...
## 
 7 thoughts on &ldquo;Concrete problems in AI safety&rdquo; 
...
If you squint, this almost reads like an article about raising kids. :-)
...
- 
 ...
 Pingback: Concrete Problems in AI Safety - Full-Stack Feed 
...
Nice write up. Robert Miles is doing a series on YouTube covering this paper as well. The link follows.
...
- 
 ...
 Pingback: Lie, damned lies, and (bad) AIs - Notes from Cloud City 
 
 
- 
 
 Pingback: The surprising creativity of digital evolution | the morning paper 
 
 
- 
 
 Pingback: The surprising creativity of digital evolution - Tech and Design 
