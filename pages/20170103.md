---
blog_url: https://blog.acolyer.org/2017/01/03/matching-networks-for-one-shot-learning/
blog_title: Matching networks for one shot&nbsp;learning
---
...
## Post navigation
 ...
 &lsaquo; PreviousInteraction networks for learning about objects, relations and physicsNext &rsaquo;Learning to learn by gradient descent by gradient descent
...
## 
 9 thoughts on &ldquo;Matching networks for one shot learning&rdquo; 
...
I think the whole point of the paper is to train incrementally with very less data. That kind of hints at not having support sets too large (equivalent to the situation a human has).
...
I am also confused by "80 classes were used for training, and testing was done on the other 20". The paper does not make it clear the problem they want to solve. They also did not compare their method with conventional one-shot learning methods. If you read the NIPS reviews, several reviewers criticize the paper is very unclear. It is amazing such a paper can be published on NIPS. Disappoint.
...
Can you give the link and reviewer number? I'll have a look
...
- 
 ...
 June 28, 2017 at 5:39 am 

 

 
 
The review cna be found here: https://media.nips.cc/nipsbooks/nipspapers/paper_files/nips29/reviews/1804.html
...
- 
 ...
 Pingback: ลุงตู่บิ๊กป้อมหมอธีร์ กับ One Shot Learning ไปถึง Siamese network(Deep Convolution Neural Network Model) นิวรอนเสี่ยงคุก!!! - 
...
This paper gives an impressively successful combination of techniques, but fails a bit to pay tribute to its predecessors. For the reference, equation (1) is refered to as Shepard's method in "Lifelong Learning Algorithms" by Thrun (1998), the idea to read one-shot training samples as a sequence is I believe introduced by "Learning to Learn Using Gradient Descent" by Hochreiter (2001).
...
Thanks for your article. I helps me a lot to understand the paper.

I have a question though: A support set S have K examples per label, and increasing K helps improve the model performance. This confuses me because I thought a support set should have only 1 example per label. Why? If it has more than 1 examples per label class, the attention might not make much sense because it will give more attention to an example in a label class and give less attention to the others in a label class. I mean, I thought the goal of attention mechanism is to give more attention to a specific label class and less attention to other label classes instead.
