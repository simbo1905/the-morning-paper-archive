---
blog_url: https://blog.acolyer.org/2016/02/04/crash-recoverability/
blog_title: Reducing Crash Recoverability to&nbsp;Reachability
---
...
Consider all of the possible traces [[P]] of a program P. Let P&#770; be a program that is identical to P except that it may crash at any point and be re-executed. We want to show that [[P&#770;]] &sube; [[P]] modolu some simulation relation. This simulation relation &#8829; relates states in P to states in P&#770;. Using &sigma; to represent a program state, we define &sigma; &#8829; &sigma;&prime; to mean that the states &sigma; and &sigma;&prime; are observationally equivalent. In other words, from an external perspective we can't tell the difference between an execution of the recovered program, and an execution of the original (un-crashed program). It doesn't have to simulate exactly what the first execution did (pre-crash), but it must simulate some feasible execution of the program.
...
Recovery proofs take the following form. Consider a program that moves through states &sigma;0 &rarr; &sigma;1 &rarr; &sigma;2 &rarr; ... &rarr; &sigma;n, at which point it crashes. The crash transitions the program to some state &sigma;&prime; from which recovery is needed. A correct recovery eventually leads to some state &sigma;&#772;k, which is observationally equivalent to some state &sigma;k in the original trace (&sigma;k &#8829; &sigma;&#772;k). Hence forward, the system behaves as if it had not crashed.
...
Using R0 to represent the program that can recover from zero crashes (true by definition) then inductive and co-inductive rules are shown for programs that can recovery from N crashes (RN, or N-recoverability) and an infinite number of crashes (&infin;-recoverability).

To automate discovering proofs of recoverability an input program is transformed into a control flow automaton &Alpha; (Eleven82 is built upon the CPAchecker tool). This input automation is then further transformed (see the paper for details) into a new automation written &Alpha;&epsilon;D. This new automaton includes a special state qerr. It is possible to reach this state if it is possible for crash-recovery to diverge or lead to a state that is not observationally equivalent to some prefix state in the original program execution. Therefore, a non-reachability proof for &Alpha;&epsilon;D entails crash recoverability of &Alpha. Hence the title of the paper, "_Reducing crash recoverability to reachability."
...
## Post navigation
 ...
 &lsaquo; Previous'Cause I'm Strong Enough: Reasoning About Consistency Choices in Distributed SystemsNext &rsaquo;Is Sound Gradual Typing Dead?
...
## 
 One thought on &ldquo;Reducing Crash Recoverability to Reachability&rdquo; 
...
- 
 ...
 Pingback: All File Systems are Not Created Equal: On the Complexity of Crafting Crash Consistent Applications | the morning paper 
