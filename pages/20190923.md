---
blog_url: https://blog.acolyer.org/2019/09/23/the-secret-sharer/
blog_title: The secret-sharer: evaluating and testing unintended memorization in neural&nbsp;networks
---
...
Given access to the model and perplexity rankings for all possible values, consider an adversary trying to guess a secret: the best strategy is to guess in rank order (of decreasing likelihood). The exposure of a secret is then determined as
 ...
 where is the space from which the random secret is drawn, and is the secret in question from that space.
