---
blog_url: https://blog.acolyer.org/2015/08/18/the-dataflow-model-a-practical-approach-to-balancing-correctness-latency-and-cost-in-massive-scale-unbounded-out-of-order-data-processing/
blog_title: The Dataflow Model: A Practical Approach to Balancing Correctness, Latency, and Cost in Massive-Scale, Unbounded, Out-of-Order Data&nbsp;Processing
---

The Dataflow Model: A Practical Approach to Balancing Correctness, Latency, and Cost in Massive-Scale, Unbounded, Out-of-Order Data Processing - Akidau et al. (Google) - 2015
...
- Timestamps are dropped since only the window is relevant from this point.

- (value, window) tuples are grouped by key

- The set of currently buffered windows for a key are merged (the merge logic being defined by the windowing strategy). For example, a sessions windowing strategy may merge windows into a new, larger session.

- Post-merging, for each resulting window the values are grouped by key

- Each per-key, per-window group of values is expanded into (key, value, event_time, window) where event_time is a new per-window timestamp - for example, the end of the window.
...
## Post navigation
 ...
 &lsaquo; PreviousLasp: A language for distributed, coordination-free programmingNext &rsaquo;Asynchronous Distributed Snapshots for Distributed Dataflows
...
## 
 10 thoughts on &ldquo;The Dataflow Model: A Practical Approach to Balancing Correctness, Latency, and Cost in Massive-Scale, Unbounded, Out-of-Order Data Processing&rdquo; 
...
- 
 ...
 Pingback: Asynchronous Distributed Snapshots for Distributed Dataflows | the morning paper 
 
 
- 
 
 Pingback: MillWheel: Fault-Tolerant Stream Processing at Internet Scale | the morning paper 
...
- 
 ...
 Pingback: Канадский геймдев -- Episode 0059 &laquo; DevZen Podcast 
...
- 
 ...
 Pingback: Time evolving graph processing at scale | the morning paper 
