---
blog_url: https://blog.acolyer.org/2018/04/19/darwin-a-genomics-co-processor-provides-up-to-15000x-acceleration-on-long-read-assembly/
blog_title: Darwin: a genomics co-processor provides up to 15,000x acceleration on long read&nbsp;assembly
---
...
The sequence alignment problem takes a query sequence Q of letters in (corresponding to the four nucleotide bases) and tries to find the best match (alignment) with a reference sequence R, assigning each letter in R and Q to either a single letter in the opposite sequence, or to a gap. The scoring scheme rewards matching bases, and penalises mismatches and gaps. For example:
...
In the figure above, the X axis corresponds to positions in R, divided into bins, each B bases wide. The Y axis corresponds to positions in Q. Ten candidate seeds from Q with length k=4 are used from consecutive positions, starting at position 1. Red dots mark the position of each hit in R, with the ‘tail’ of the dot covering k=4 bases in R and Q showing the extent of the match.
...
Seed lookup is implemented using a seed position table: for each of the possible seeds, a seed pointer table points to the beginning of a list of hits in a position table.
...
> 
 ...
 A linear array of processing elements (PEs) exploits wavefront parallelism to compute up to cells of the DP-matrix for the Smith-Waterman algorithm with affine gap penalties. Each clock cycle, each PE computes three scores and one trackback pointer for one cell of the DP-matrix.
...
## Post navigation
 ...
 &lsaquo; PreviousGoogle workloads for consumer devices: mitigating data movement bottlenecksNext &rsaquo;The architectural implications of autonomous driving: constraints and acceleration
...
## 
 7 thoughts on &ldquo;Darwin: a genomics co-processor provides up to 15,000x acceleration on long read assembly&rdquo; 
...
- 
 ...
 Pingback: New top story on Hacker News: Darwin: a genomics co-processor with upto 15000x acceleration - Tech + Hckr News 
 
 
- 
 
 Pingback: New top story on Hacker News: Darwin: a genomics co-processor with upto 15000x acceleration - Technical Nair 
 
 
- 
 
 Pingback: New top story on Hacker News: Darwin: a genomics co-processor with upto 15000x acceleration - Latest news 
 
 
- 
 
 Pingback: Darwin: a genomics co-processor with upto 15000x acceleration - posted at April 19, 2018 at 08:43AM by godelmachine - Startup News 2018 
 
 
- 
 
 Pingback: New top story on Hacker News: Darwin: a genomics co-processor provides up to 15,000x acceleration - ÇlusterAssets Inc., 
 
 
- 
 
 Pingback: Darwin: a genomics co-processor supplies up to fifteen,000x acceleration on prolonged read meeting | A1A 
