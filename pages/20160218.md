---
blog_url: https://blog.acolyer.org/2016/02/18/ownership-and-reference-counting-based-garbage-collection-in-the-actor-world/
blog_title: Ownership and Reference Counting Based Garbage Collection in the Actor&nbsp;World
---
...
- Reference counts are always &ge; 0.

- If an object &omega; is in the heap of some actor &alpha;, then the owner of &omega;, &alpha;o, is also in the heap of &alpha; 

- If there is a message in a queue containing an address i, then the local reference count for i (at i's owning actor) is > 0.

- If some actor &alpha; can reach an address that it does not own, then both the owners local reference count and the &alpha;'s (foreign) reference count for that address are &ge; 0.
...
- The actor has no path leading to the object

- The local reference count for the object is 0
...
- Its local reference count for itself is zero
...
- All owned objects are marked unreachable.

- All unowned objects with a foreign reference count greater than zero are marked as unreachable.

- Tracing occurs from the actor’s fields only, marking objects reachable, whether they are owned or not.

- All owned objects with a local reference count greater than zero are marked as reachable.

- Owned objects that are locally unreachable and have a local reference count of zero collected.

- Decrement messages are sent for unowned objects that are unreachable, and their foreign reference count is set to 0.
...
## Post navigation
 ...
 &lsaquo; PreviousDeny Capabilities for Safe, Fast ActorsNext &rsaquo;Formal Requirements for Virtualizable Third Generation Architectures
