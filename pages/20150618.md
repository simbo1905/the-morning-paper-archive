---
blog_url: https://blog.acolyer.org/2015/06/18/spinning-fast-iterative-dataflows/
blog_title: Spinning Fast Iterative&nbsp;Dataflows
---
...
- The step function &Delta; must consist solely of record-at-a-time operations (e.g. Map) such that record is processed individually.

- Binary operators in &Delta; must have at most one input on the dynamic data path
...
- To enable parallelism without fine-grained distributed locking, the dataflow between S and D must not cross partition boundaries, which can be inferred from &Delta;:
...
## Post navigation
 ...
 &lsaquo; PreviousDifferential DataflowNext &rsaquo;Discretized Streams: Fault Tolerant Stream Computing at Scale
...
## 
 3 thoughts on &ldquo;Spinning Fast Iterative Dataflows&rdquo; 
...
- 
 ...
 Pingback: Monday Cool Links | /dev 
...
Thanks for this post, can you tell me the difference between microstep and superstep?. My understanding is that, both microstep and superstep produces partial solution in each iteration, but in the case of superstep the partial solution is produces using entire w, where as in microstep, partial solution is produced by considering only 1 item from w, thereby allowing/making it easy to make the processing parallel. Am I correct?
