---
blog_url: https://blog.acolyer.org/2016/12/14/scone-secure-linux-containers-with-intel-sgx/
blog_title: SCONE: Secure Linux containers with Intel&nbsp;SGX
---
...
One limitation (due to SGX itself) is that SCONE cannot support fork, exec, and clone. When system calls are made, unless special support is provided a micro-benchmark showed that the enclave adds an order-of-magnitude overhead. To mitigate this, SCONE uses an asynchronous system call interface:
...
- A console shield protects the confidentiality of data sent via the stdin, stdout, and stderr streams.
...
## Post navigation
 ...
 &lsaquo; PreviousDiamond: Automating data management and storage for wide-area, reactive applicationsNext &rsaquo;FaSST: Fast, scalable and simple distributed transactions with two-sided (RDMA) datagram RPCs
...
## 
 13 thoughts on &ldquo;SCONE: Secure Linux containers with Intel SGX&rdquo; 
...
Can Intel ME bypass or override SGX?
...
- 
 ...
 Pingback: Secure Linux containers with Intel SGX | Firmware Security 
 
 
- 
 
 Pingback: Panoply: Low-TCB Linux applications with SGX enclaves | the morning paper 
...
[...] SCONE: Secure Linux containers with Intel SGX [...] [...] Previously we've looked at Haven, which uses SGX in the context of cloud infrastructure, SCONE which shows how to run docker containers under SGX, and Panoply which looks at what happens when [...]
...
- 
 ...
 Pingback: SGXBounds: memory safety for shielded execution | the morning paper 
...
[...] Previously we've looked at Haven, which uses SGX in the context of cloud infrastructure, SCONE which shows how to run docker containers under SGX, and Panoply which looks at what happens when [...] [...] SCONE: Secure Linux containers with Intel SGX [...]
