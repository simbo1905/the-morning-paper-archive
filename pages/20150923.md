---
blog_url: https://blog.acolyer.org/2015/09/23/firefly-algorithms-for-multi-model-optimization/
blog_title: Firefly Algorithms for Multimodal&nbsp;Optimization
---
...
Since attractiveness &beta; is proportional to light intensity we can therefore use:

&beta;( r ) = &beta;0e-&gamma;r2

And to avoid calculating an exponential function, we can replace this with:

&beta;( r ) = &beta;0/(1 + &gamma;r2)

The distance between fireflies is just the Cartesian Distance, which we can extend into multiple dimensions as needed. Putting all this together with a randomisation parameter &alpha; gives us the equation for moving fireflies on each iteration, firefly i moves towards a brighter firefly j according to:

xi = xi + &beta;0e-&gamma;r2ij(xj - xi) + &alpha;(rand - 1/2)
...
When &gamma; &rarr; 0 we have the situation where light intensity does not decrease with distance in an idealized sky, and this corresponds to Particle Swarm Optimisation. When &gamma; &rarr; &infin; we have 'very shortsighted fireflies' ! This corresponds to the completely random search method.
...
## Post navigation
 ...
 &lsaquo; PreviousAnt Algorithms for Discrete OptimizationNext &rsaquo;A New Metaheuristic Bat-Inspired Algorithm
