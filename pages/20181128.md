---
blog_url: https://blog.acolyer.org/2018/11/28/towards-usable-checksums-automating-the-integrity-verification-of-web-downloads-for-the-masses/
blog_title: Towards usable checksums: automating the integrity verification of web downloads for the&nbsp;masses
---
...
- For the 70.6% of respondents that do sometimes download something, 58.6% declared downloading from developer’s websites, and 42.6% use P2P networks at least once a year.
...
The authors also studied the behaviour of participants when downloading files using the extension.

- 40% of the participants stopped when shown the warning message for the corrupted download. For those that didn’t, most said that they tend to ignore popups.

- For those that did stop, 50% went on to remove the downloaded file, including (37.5%) via the embedded ‘delete’ link in the warning message.

The learnings from this part of the experiment are that the extension is useful, but the warnings could be improved and the deletion of a corrupted file should be automatic.

> 

 We argue that designers should choose defaults to privilege users’ security over preserving user’s choice. In fact, a lack of action from the user does not necessarily mean that she is willing to accept the risk of proceeding but might simply be due to the aforementioned effects &#091;lack of understanding of the purpose of checksums etc.&#093;.

The authors now intend to deploy an improved beta version of the software and conduct a longitudinal study. As well as the Chrome extension for downloaders, the authors also built a WordPress extension to help file hosters automatically generate and include checksums.

> 

 In addition, we intend to promote our proposals to (and collaborate with) the different stakeholders involved, that is the W3C, web browsers (e.g. Google, Mozilla) and CMS (e.g WordPress, Drupal) development teams in order to have a concrete impact on the security of Internet users.
...
## Post navigation
 ...
 &lsaquo; PreviousBEAT: asynchronous BFT made practicalNext &rsaquo;LEMNA: explaining deep learning based security applications
...
## 
 7 thoughts on &ldquo;Towards usable checksums: automating the integrity verification of web downloads for the masses&rdquo; 
...
I skimmed the paper but didn't see the Chrome Web Store link for the extension. Do you happen to have it, or was I just missing something obvious? Thanks!
...
I don't get it — why do you need this is if you're using HTTPS? Presumably, this would stop any attacker from messing with the traffic?
...
HTTPS would prevent the man in the middle attack but the file could be tampered on the website itself. HTTPS doesn't prevent that.
...
OK, but if you assume the attacker can tamper with the website, can't they change the displayed checksum too?
...
That’s why another of the recommendations is to host the checksum and the download on separate sites ;)
...
Definitely. That's why the paper talks about hosting them in different domain.
...
We’ve come up with an automated solution for verifying the integrity of web downloads, which does not require a browser plug-in. The solution verifies the integrity of the downloadable file using a javascript function, which originates from the same server that the trusted checksum originates from. For more information, see https://www.meixler-tech.com/aivwd/
