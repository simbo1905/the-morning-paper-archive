---
blog_url: https://blog.acolyer.org/2019/05/10/distributed-consensus-revised-part-iii/
blog_title: Distributed consensus revised &#8211; Part&nbsp;III
---
...
> 
A key implication of this result is that for the minimum epoch there is no phase one quorum intersection requirement. The practical application of this is that a proposer with epoch may skip phase one and proceed directly to proposing their own value in phase two.
...
If a proposer with epoch receives a proposal in a promise response , then it has learned that if a decision was reached in epoch , then the value chosen was . This allows us to further weaken the quorum intersection requirement (‘revision C’): the proposer no longer needs no intersect with the phase two quorums for epochs up to and including , but must continue to intersect with any phase two quorums for epochs greater than (up to of course, its current epoch).
...
Ignoring epochs for the moment, the first idea is to consider each possible quorum, and whether or not that quorum could have reached a decision. If no quorum could have reached a decision then we’re free to propose our own value. For example, let the set of acceptors be and the possible quorum sets be and . A proposer in epoch 5 receives the following promises in order:
...
With Classic Paxos the proposer would have to propose A (the value associated with the highest epoch). But here we know that A cannot have been decided because the nil response from makes that impossible. Since we have a returned proposal (3, _), we also know that the proposal (2, B) from cannot have been successful.
...
## Post navigation
 ...
 &lsaquo; PreviousDistributed consensus revised - Part IINext &rsaquo;An open-source benchmark suite for microservices and their hardware-software implications for cloud & edge systems
...
## 
 One thought on &ldquo;Distributed consensus revised - Part III&rdquo; 
...
With a strict majority using 4 nodes is slower than 3 nodes without any additional resilience. Likewise 6 nodes is slower than 5 with no additional resilience. However you can commit on 50% accept responses as that will overlap with a majority propose response. In steady state a 4 or 6 node cluster is then no slower than one less node and you get an additional hot spare. Applying this optimisation to a multi-paxos implementation is a single line code change with a big impact. I wrote a blog about this optimisation at https://simbo1905.blog/2016/09/30/the-fpaxos-even-nodes-optimisation/
