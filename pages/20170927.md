---
blog_url: https://blog.acolyer.org/2017/09/27/an-efficient-bandit-algorithm-for-real-time-multivariate-optimization/
blog_title: An efficient bandit algorithm for real-time multivariate&nbsp;optimization
---
...
We can represent a layout A as a D-dimensional vector, where denotes the content chosen for the widget. X and_A_ can be combined (possibly non-linearly) to form a feature vector .

The reward for a given layout and context depends on some linear scaling of by a fixed but unknown vector of weights .

This all sounds quite fancy. But all we've really got going on here is a system of the form:

 

Where etc, are constants (weights) in the vector , are the context variables, and are the layout variables.

> 

 We now define a stochastic contextual multi-armed bandit problem. We have N^D arms, one per layout.

At each time step , we select an arm and observe the reward . We always have a best-so-far arm, and the overall aim is to estimate the coefficients while minimising the regret we feel from sometimes choosing an arm that isn't as good as the best-so-far.
...
## Post navigation
 ...
 &lsaquo; PreviousSeven rules of thumb for web site experimentersNext &rsaquo;Peeking at A/B tests: continuous monitoring without pain
...
## 
 4 thoughts on &ldquo;An efficient bandit algorithm for real-time multivariate optimization&rdquo; 
...
- 
 ...
 Pingback: Amazon 的多變數最佳化 | Gea-Suan Lin&#039;s BLOG 
 
 
- 
 
 Pingback: An efficient bandit algorithm for real-time multivariate optimization | Indri Digital 
